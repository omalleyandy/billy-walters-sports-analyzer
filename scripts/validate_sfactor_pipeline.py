...
class ValidationReport:
    ...
    def add_test(self, name: str, passed: bool, details: Dict[str, Any]):
        """Add test result"""
        self.results["tests"][name] = {"passed": passed, "details": details}
    ...
    def generate_summary(self):
        ...
        self.results["summary"] = {
            ...
            "critical_issues": sum(
                1 for i in self.results["issues"] if i["severity"] == "CRITICAL"
            ),
            "warnings": sum(
                1 for i in self.results["issues"] if i["severity"] == "WARNING"
            ),
        }
    ...
    def print_console_report(self):
        ...
        if self.results["summary"]["failed_tests"] == 0:
            ...
            print(
                f"[ERROR] {self.results['summary']['failed_tests']} tests failed - Review needed"
            )
        print("=" * 70 + "\n")
...
def validate_team_context_builder():
    ...
    try:
        ...
        passed = (
            buf_context.team_name == "Buffalo Bills"
            and buf_context.abbreviation == "BUF"
            and buf_context.current_power_rating == 7.5
            and buf_context.quality_tier.value == "elite"
            and buf_context.total_injury_impact == -0.5
        )
    ...
    try:
        ...
        passed = (
            hou_context.total_injury_impact == -7.5
            and adjusted_rating == -5.5
            and hou_context.key_injuries_count == 1
        )
    ...
    try:
        ...
        passed = (
            kc_context.total_injury_impact == 0.0
            and kc_context.key_injuries_count == 0
            and kc_context.quality_tier.value == "elite"
        )
...
def validate_schedule_calculator():
    ...
    try:
        history = calculator.calculate_schedule_history(
            team_abbr="BUF", current_date=date(2025, 11, 21), recent_games=buf_schedule
        )

        # Buffalo to Houston is ~1,200 miles
        passed = (
            history.total_games == 1
            and history.total_travel_miles > 1000
            and history.primetime_games_count == 1
            and history.days_since_last_game == 7
        )
    ...
    try:
        ...
        passed = history.days_since_last_game == 4 and history.has_short_week is True
...
def main():
    ...
    final_report.results["issues"] = (
        team_report.results["issues"] + schedule_report.results["issues"]
    )
