...
Examples:
  ...
  %(prog)s --output data/odds        # Save to custom directory
    ...
    parser.add_argument(
        "--proxy", type=str, help="Proxy URL (format: http://user:pass@host:port)"
    )
...
async def main():
    ...
    try:
        ...
        timestamp = (
            overtime_data["scrape_metadata"]["timestamp"]
            .replace(":", "-")
            .replace(".", "-")
        )
        ...
        if args.convert:
            ...
            if args.save_db:
                ...
                try:
                    ...
                    print(
                        "[WARNING] Database module not found - skipping database save"
                    )
