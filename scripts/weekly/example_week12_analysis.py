...
def analyze_week12_game():
    ...
    print("-" * 70)
    print(
        f"Home (Panthers): Time Zone={get_team_time_zone(home_team)}, "
        f"Warm Weather={is_warm_weather_team(home_team)}, "
        f"Dome={is_dome_team(home_team)}"
    )
    print(
        f"Away (Chiefs):   Time Zone={get_team_time_zone(away_team)}, "
    ...
    adjusted_predicted_spread = (
        base_predicted_spread - net_total
    )  # Subtract because it's Panthers advantage

    print(
        f"Base Predicted Spread (from power ratings): Panthers {base_predicted_spread:+.1f}"
    )
    print(f"S/W Factor Adjustment:                      {net_total:+.2f} to Panthers")
    print(
        f"Adjusted Predicted Spread:                  Panthers {adjusted_predicted_spread:+.1f}"
    )
    ...
    if edge > 0:
        ...
        print("RECOMMENDATION: No bet (line is fair)")
    ...
    print(
        f"This game S-Factor: {panthers_sf.total_points + chiefs_sf.total_points:.1f} points = "
