...
class CLVTracker:
    ...
    def update_closing_line(self, bet_id: int, closing_line: float):
        ...
        for bet in bets:
            if int(bet["bet_id"]) == bet_id:
                ...
                clv_points = self._calculate_clv_points(
                    bet_line, closing_line, bet["team"], bet["game"]
                )
                clv_percentage = (
                    (abs(clv_points) / abs(closing_line)) * 100
                    if closing_line != 0
                    else 0
                )
                ...
                if clv_points > 0:
                    print("   🎯 BEATING CLOSING LINE! Positive CLV!")
                elif clv_points < 0:
                    print("   ⚠️ Worse than closing - negative CLV")
                else:
                    print("   ➖ Equal to closing")
    ...
    def summary(self):
        ...
        if bets_with_clv:
            ...
            print("\n🎯 CLV Performance:")
            print(
                f"   Beating Closing Line: {positive_clv}/{len(clv_values)} ({clv_percentage:.1f}%)"
            )
            print(f"   Average CLV: {avg_clv:+.2f} points")

            if clv_percentage > 55:
                print("   ✅ EXCELLENT! Above 55% target")
            elif clv_percentage > 50:
                print("   ✅ GOOD! Positive CLV trend")
            else:
                print("   ⚠️ Below 50% - review line shopping")
        ...
        if completed_bets:
            ...
            profit = sum(
                int(b["amount"]) * 0.909 for b in completed_bets if b["result"] == "won"
            )
            loss = sum(
                int(b["amount"]) for b in completed_bets if b["result"] == "lost"
            )
        ...
        for bet in bets[-5:]:
            ...
            print(
                f"   #{bet['bet_id']} {result_emoji} {bet['team']} {bet['bet_line']} ${bet['amount']} CLV:{clv_str}"
            )
    ...
    def list_pending(self):
        ...
        for bet in pending:
            ...
            if bet["closing_line"]:
                ...
                print("   Closing: Not yet updated")
            print(f"   Placed: {bet['placed_time']}")

        print("=" * 60 + "\n")

    def _calculate_clv_points(
        self, bet_line: float, closing_line: float, team: str, game: str
    ) -> float:
...
def main():
    parser = argparse.ArgumentParser(
        description="Simple CLV Tracker for Billy Walters System"
    )
    ...
    record.add_argument(
        "--line", required=True, type=float, help="Line you got (e.g., +3.5, -6.5)"
    )
    record.add_argument(
        "--amount", required=True, type=int, help="Bet amount in dollars"
    )
    record.add_argument(
        "--edge", type=float, default=0.0, help="Calculated edge percentage"
    )
    ...
    closing.add_argument(
        "--closing-line", required=True, type=float, help="Final closing line"
    )
    ...
    result.add_argument(
        "--result", required=True, choices=["won", "lost", "push"], help="Bet result"
    )
    ...
    if args.command == "record":
        tracker.record_bet(
            args.game, args.team, args.line, args.amount, args.edge, args.notes
        )
