...
CLI script to fetch college football team news articles from ESPN blog pages.
Extracts article titles, dates, content summaries, and categories for team
strength analysis.

Usage:
    # Get news for one team
    uv run python scripts/scrapers/scrape_espn_ncaaf_news.py --team alabama

    # Get news for multiple teams
    uv run python scripts/scrapers/scrape_espn_ncaaf_news.py --teams alabama lsu georgia

    # Get news for all major teams
    uv run python scripts/scrapers/scrape_espn_ncaaf_news.py --all-teams

    # Get more articles per team
    uv run python scripts/scrapers/scrape_espn_ncaaf_news.py --all-teams --limit 20

    # Run in visible browser (for debugging)
    uv run python scripts/scrapers/scrape_espn_ncaaf_news.py --team alabama --no-headless
...
from data.espn_ncaaf_news_client import ESPNNCAAFNewsClient, NCAAF_TEAMS


async def main():
    """Main CLI entry point."""
    parser = argparse.ArgumentParser(
        description="ESPN NCAAF News & Blog Scraper",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Get news for Alabama
  %(prog)s --team alabama

  # Get all SEC Teams news
  %(prog)s --teams alabama lsu georgia

  # Get all major FBS teams news
  %(prog)s --all-teams

  # Save to custom location
  %(prog)s --all-teams --output-dir news/ncaaf
  ...
  %(prog)s --all-teams --limit 20
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("output/espn/news/ncaaf"),
        help="Output directory. Default: output/espn/news/ncaaf",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
    ...
    try:
        if not args.quiet:
            print("[*] ESPN NCAAF News & Blog Scraper")
            print("=" * 70)
        ...
        if not args.quiet:
            print("\n" + "=" * 70)
            print("NCAAF NEWS SCRAPING COMPLETE")
            print("=" * 70)
