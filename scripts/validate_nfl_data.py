...
class NFLDataValidator:
    ...
    def __init__(self):
        ...
        self.valid_teams = {
            ...
            # AFC West
            "Denver", "Kansas City", "Las Vegas", "LA Chargers",
            # NFC East
    ...
    def validate_games_file(self, games_file: Path) -> bool:
        ...
        if 'simulated' in source.lower() or 'placeholder' in source.lower():
            ...
            print(f"  [ERROR] Data appears to be simulated/fake")
        ...
        for i, game in enumerate(games):
            ...
            if home_score > 70 or away_score > 70:
                self.warnings.append(f"Week {week}: {away_team} @ {home_team} ({away_score}-{home_score}) - Unusually high score")
    ...
    def validate_team_records(self, games_file: Path) -> Dict[str, Dict]:
        ...
        games = data['games']
        teams = {team: {'wins': 0, 'losses': 0, 'pf': 0, 'pa': 0} for team in self.valid_teams}
        ...
        for i, (team, record) in enumerate(sorted_teams[:5], 1):
            ...
            print(f"    {i}. {team:20s} {record['wins']}-{record['losses']}  ({diff:+4d} diff)")
        ...
        for team, record in teams.items():
            ...
            if total_games >= 9 and record['losses'] == 0:
                self.warnings.append(f"{team} is undefeated ({record['wins']}-0) - verify data accuracy")

            # Check for winless teams
            if total_games >= 9 and record['wins'] == 0:
                self.warnings.append(f"{team} is winless (0-{record['losses']}) - verify data accuracy")

            # Check for extreme point differentials
            diff = record['pf'] - record['pa']
            if abs(diff) > 200:
                self.warnings.append(f"{team} has extreme point differential ({diff:+d}) - verify data accuracy")
    ...
    def validate_power_ratings(self, ratings_file: Path, team_records: Dict) -> bool:
        ...
        for team, rating in sorted_ratings[:10]:  # Top 10 rated teams
            ...
            if losses > wins:
                misalignments.append(f"{team}: Top 10 rating ({rating:.2f}) but losing record ({wins}-{losses})")
        ...
        for team, rating in sorted_ratings[-10:]:  # Bottom 10 rated teams
            ...
            if wins > losses + 2:
                misalignments.append(f"{team}: Bottom 10 rating ({rating:.2f}) but winning record ({wins}-{losses})")
        ...
        if misalignments:
            ...
            print(f"  [OK] No significant misalignments detected")
