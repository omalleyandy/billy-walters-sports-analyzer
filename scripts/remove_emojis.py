...
def remove_emojis_from_file(filepath: Path) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        if remaining_emojis:
            ...
            changes.append(
                f"Removed {len(remaining_emojis)} other emojis ({len(unique_emojis)} unique)"
            )
            if len(unique_emojis) <= 5:  # Show which ones if not too many
                changes.append(
                    f"  Removed: {', '.join(repr(e) for e in unique_emojis)}"
                )
...
def process_project(project_root: Path, dry_run: bool = False) -> None:
    ...
    print(f"\n{'=' * 70}")
    print("  Emoji Removal Tool - Windows Compatibility (Fixed)")
    print(f"{'=' * 70}")
    print(
    ...
    for filepath in sorted(python_files):
        if dry_run:
            ...
            try:
                ...
                has_emojis = any(
                    emoji in content for emoji in EMOJI_REPLACEMENTS.keys()
                )
                ...
                if has_emojis or has_other_emojis:
                    ...
                    for emoji, replacement in EMOJI_REPLACEMENTS.items():
                        ...
                        if count > 0:
                            print(
                                f"  - Would replace {count}x '{emoji}' with '{replacement}'"
                            )
                    if has_other_emojis:
                        print("  - Would remove other Unicode emojis")
                    modified_count += 1
            except Exception as e:
    ...
    if dry_run:
        ...
        print("  python scripts/remove_emojis.py")
    else:
        ...
        if modified_count > 0:
            ...
            print("2. Test scripts: python check_status.py")
            print("3. Run a script to verify: python analyze_week12.py")
    print(f"{'=' * 70}\n")
