...
print(f"Sharp plays (5+ div): {len(nfl_divs)} ({len(nfl_divs)/nfl['game_count']*100:.0f}%)")
...
print(f"Sharp plays (5+ div): {len(ncaaf_divs)} ({len(ncaaf_divs)/ncaaf['game_count']*100:.0f}%)")
print(f"Max divergence: {max(ncaaf_divs) if ncaaf_divs else 0}")
print(f"Avg divergence: {sum(ncaaf_divs)/len(ncaaf_divs):.1f}" if ncaaf_divs else "N/A")
...
for p in ncaaf['sharp_plays']:
    if p['divergence'] >= 30:
        ...
        print(f"    Tickets: {p['tickets_pct']}% | Money: {p['money_pct']}% | Div: +{p['divergence']}")
