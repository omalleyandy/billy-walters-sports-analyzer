...
for game_name, data in games.items():
    ...
    if data["overtime_line"] is None:
        ...
        print("   [*] Go to overtime.ag/sports#/nfl")
        print(f"   [*] Find {game_name}")
        print("   [*] Fill in overtime_line value in this script")
...
if qualified_bets:
    ...
    print(f"\n{'=' * 80}")
    print(
        f"[MONEY] TOTAL RISK: ${total_risk:.0f} ({total_risk / BANKROLL * 100:.1f}% of bankroll)"
    )

    if total_risk / BANKROLL <= 0.15:
        print("[*] Within 15% weekly limit")
    else:
        print("[WARNING]  EXCEEDS 15% weekly limit - reduce bet sizes!")

    print("\n[NOTE] NEXT STEPS:")
    print("   1. Screenshot this output for records")
    print("   2. Go to overtime.ag/sports#/nfl")
    print("   3. Place bets in priority order (HIGH first)")
    print("   4. Verify lines haven't moved significantly")
    print("   5. Take confirmation screenshots")
...
if not qualified_bets and not waiting_for_lines:
    ...
    print("   • Better to wait than force bets")
    print(
        "   • Billy Walters: 'If you don't run out of money, you won't run out of things to bet on'"
    )
