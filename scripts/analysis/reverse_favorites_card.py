...
for i, play in enumerate(better_teams, 1):
    weather_tag = f" [{play['weather']}]" if play['weather'] in ["HIGH", "EXTREME"] else ""
    team_rating = play['away_rating'] if play['location'] == 'away' else play['home_rating']
    opp_rating = play['home_rating'] if play['location'] == 'away' else play['away_rating']

    print(f"{i}. {play['play']} ({play['time']}) {weather_tag}")
    print(f"   Quality Edge: {play['team']} is {play['quality_edge']:.2f} points BETTER")
    print(f"   Ratings: {play['team']} {team_rating:.2f} vs {play['opponent']} {opp_rating:.2f}")
    print(f"   Spread Edge: {play['spread_edge']:.1f} points")
    print(f"   WHY THEY'RE UNDERDOG: {play['location'].upper()} game, market overvaluing opponent")
    print()
...
print("TIER 1 PLAYS (Better Team): Bet with confidence - you're backing the superior team")
...
for i, play in enumerate(reverse_favs[:10], 1):
    ...
    print(f"{i}. [{tier}] {play['play']} (Quality: {play['quality_edge']:+.2f}, Spread Edge: {play['spread_edge']:.1f})")
