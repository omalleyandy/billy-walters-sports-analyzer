...
def main():
    ...
    top_5 = sorted(backtest.initial_ratings.items(), key=lambda x: x[1], reverse=True)[
        :5
    ]
    ...
    for i, (team, change) in enumerate(result.biggest_movers[:5], 1):
        ...
        print(
            f"  {i}. {team:<25} {initial:.2f} -> {final:.2f} ({change:+.2f}) {direction}"
        )
    print()

    # Best week
    if result.weekly_stats:
        best_week = max(result.weekly_stats.items(), key=lambda x: x[1]["ats_accuracy"])
