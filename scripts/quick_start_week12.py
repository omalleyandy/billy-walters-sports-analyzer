...
def main():
    ...
    parser.add_argument('--week', type=int, default=12, help='Week number (default: 12)')
    parser.add_argument('--season', type=int, default=2025, help='Season year (default: 2025)')
    parser.add_argument('--bankroll', type=float, default=20000.0, help='Bankroll amount (default: 20000)')
    parser.add_argument('--force-new', action='store_true', help='Force create new session')
    ...
    if not args.force_new:
        ...
        if existing and existing.season == args.season:
            ...
            print(f"     Last Updated: {existing.last_updated.strftime('%Y-%m-%d %H:%M:%S')}")
            ...
            if existing.total_risk_deployed > 0:
                ...
                print(f"     Risk Deployed: ${existing.total_risk_deployed:,.2f} ({risk_pct:.1f}%)")
            ...
            print(f"[INFO] Creating new session...")
            session = session_mgr.create_session(args.week, args.season, args.bankroll)
    else:
        print(f"[INFO] Force creating new session...")
        session = session_mgr.create_session(args.week, args.season, args.bankroll)
    ...
    checklist_items = [
        ("Power ratings updated (Week 11 results applied)", check_power_ratings(session), "CRITICAL"),
    ...
    print(f"Total Deployed: ${risk_summary['total_deployed']:,.2f} ({risk_summary['deployment_pct']:.1f}%)")
    ...
    if session.opportunities_identified == 0:
        ...
        print("    Command:")
        print(f"    python analyze_edges.py --week {args.week} --bankroll {args.bankroll:.0f} --min-edge 5.5")
        print()
