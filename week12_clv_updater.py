...
def show_summary():
    ...
    if with_closing > 0:
        ...
        clv_avg = sum(b.clv_points for b in bets if b.clv_points is not None) / with_closing
        
        print("="*70)
        print("CLV PERFORMANCE")
        print("="*70)
        print(f"Beat Closing:     {clv_positive}/{with_closing} ({clv_positive/with_closing*100:.1f}%)")
        ...
        for i, bet in enumerate(sorted(bets, key=lambda b: b.created_at), 1):
            ...
            beat_str = "✓" if bet.beat_closing_line else "✗" if bet.beat_closing_line == False else "?"
            result_str = "WON" if bet.did_bet_win == True else "LOST" if bet.did_bet_win == False else "PENDING"
            
            print(f"{i}. {bet.game_id}")
            print(f"   Open: {bet.opening_line:+.1f} | Close: {bet.closing_line:+.1f if bet.closing_line else 'TBD':>6} | CLV: {clv_str:>6} {beat_str}")
            print(f"   Edge: {bet.edge_percentage:.1f}% | Stake: {bet.stake_fraction*100:.1f}% (${bet.stake_fraction * bet.bankroll:.0f}) | Result: {result_str}")
            print()
    ...
    if with_results > 0:
        ...
        total_staked = sum(b.stake_fraction * b.bankroll for b in bets if b.did_bet_win is not None)
        total_profit = sum(
            (b.stake_fraction * b.bankroll * 0.91) if b.did_bet_win == True
            else (-(b.stake_fraction * b.bankroll)) if b.did_bet_win == False
            else 0
            for b in bets if b.did_bet_win is not None
        )
