...
class CLVStorage:
    ...
    def list_pending(self, limit: Optional[int] = None) -> List[CLVTracking]:
        ...
        bets = self._load_all_bets()
        pending = [
            CLVTracking(**data) for data in bets.values()
    ...
    def update_closing_line(
        ...
        # Calculate CLV
        clv_points, outcome = CLVAnalyzer.calculate_clv(
            bet.opening_line,
            closing_line
        )
    ...
    def get_summary(
        ...
        logger.info(f"Generated summary for {period_str}: {summary.clv_percentage:.1f}% CLV")
    ...
    def export_to_csv(
        ...
        with open(output_path, 'w', newline='') as f:
            fieldnames = [
                'recommendation_id', 'game_id', 'opening_line', 'closing_line',
                'final_line', 'clv_points', 'clv_outcome', 'did_bet_win',
                'beat_closing_line', 'edge_percentage', 'stake_fraction',
                'bankroll', 'bet_side', 'bet_type', 'opening_date',
                'closing_date', 'created_at', 'notes'
            ]
            ...
            for bet in bets:
                writer.writerow({
                    ...
                    'clv_outcome': bet.clv_outcome.value if bet.clv_outcome else None,
                    ...
                    'closing_date': bet.closing_date.isoformat() if bet.closing_date else None,
    ...
    def _append_to_csv(self, bet: CLVTracking) -> None:
        ...
        fieldnames = [
            ...
            'beat_closing_line', 'edge_percentage', 'stake_fraction',
            'bankroll', 'bet_side', 'bet_type', 'opening_date',
            'closing_date', 'created_at', 'updated_at', 'notes'
        ]
...
class CLVReporter:
    ...
    def generate_summary(bets: List[CLVTracking]) -> CLVSummary:
        ...
        beating_closing = sum(
            1 for b in resolved
            if b.beat_closing_line is True
        )
        ...
        gross_wins = sum(
            b.bankroll * b.stake_fraction for b in resolved
            if b.did_bet_win is True
        )
        gross_losses = sum(
            b.bankroll * b.stake_fraction for b in resolved
            if b.did_bet_win is False
        )
