...
class BillyWaltersValuation:
    ...
    def apply_injury_multiplier(
        ...
        return self.injury_calculator.calculate_injury_impact(
            player_value, injury_type, days_since_injury
        )

    def calculate_team_impact(self, injuries: List[Dict], team_name: str = "") -> Dict:
        """
        ...
        for injury in injuries:
            ...
            if tier is None:
                ...
                tier = self.player_valuation.determine_tier_from_depth_chart(
                    position, depth_chart_position=1
                )
        ...
        crises = self.market_analyzer.analyze_position_group_crisis(position_impacts)
    ...
    def analyze_game(
        ...
        # Generate response
        total_game_impact = (
            home_analysis["total_impact"] + away_analysis["total_impact"]
        )
        ...
        response = self.market_analyzer.generate_response(
            total_game_impact,
            {
                ...
                "critical_count": len(home_analysis["critical_injuries"])
                + len(away_analysis["critical_injuries"]),
                "team": home_team
                if home_analysis["total_impact"] > away_analysis["total_impact"]
                else away_team,
                "position_group": "offensive line"
                if any(
                    "O-LINE" in c
                    for c in home_analysis.get("position_group_crises", [])
                )
                else "secondary",
                ...
                "critical_players": ", ".join(
                    [
                        inj["name"]
                        for inj in (
                            home_analysis["critical_injuries"]
                            + away_analysis["critical_injuries"]
                        )[:3]
                    ]
                ),
                "injured_count": home_analysis["injury_count"]
                + away_analysis["injury_count"],
    ...
    def format_game_output(self, game_analysis: Dict) -> str:
        ...
        lines.append(
            f"Expected Line Movement: {edge['expected_line_movement']:.1f} points"
        )
