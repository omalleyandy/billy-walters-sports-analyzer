...
class InjuryImpactCalculator:
    ...
    def calculate_injury_impact(
        self, player_value: float, injury_type: InjuryType, days_since_injury: int = 0
        ...
        if injury_key not in self.injury_multipliers:
            ...
            injury_data = self.injury_multipliers.get(
                "QUESTIONABLE",
                {
                    "immediate": 0.92,
                    "recovery_days": 0,
        ...
        if injury_type == InjuryType.OUT:
            ...
            if recovery_days > 0:
                ...
                current_capacity = (
                    immediate_capacity + (1.0 - immediate_capacity) * recovery_progress
                )
            else:
            ...
            current_capacity = lingering_capacity + (1.0 - lingering_capacity) * (
                weeks_post / 2
            )
            capacity_pct = int(current_capacity * 100)
            explanation = (
                f"{injury_type.value}: Post-recovery at {capacity_pct}% capacity"
            )
        else:
            # Fully recovered
    ...
    def calculate_team_injury_impact(self, injured_players: list) -> Dict:
        """
