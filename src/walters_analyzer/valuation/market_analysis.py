...
class MarketAnalyzer:
    ...
    def calculate_betting_edge(
        ...
        adjusted_impact = self._apply_context_multipliers(
            net_injury_impact, game_context
        )
        ...
        bet_recommendation = self._generate_bet_recommendation(
            abs(edge), net_injury_impact, home_injury_impact, away_injury_impact
        )
        ...
        if edge > 0.5:
            ...
            if net_injury_impact > 0:
                ...
                reasoning = (
                    "Home team has injury advantage, line hasn't adjusted enough"
                )
            else:
                recommended_side = "away"
                reasoning = (
                    "Away team has injury advantage, line hasn't adjusted enough"
                )
        elif edge < -0.5:
            # Line moved too much
    ...
    def _apply_context_multipliers(self, base_impact: float, context: Dict) -> float:
        """
    ...
    def _generate_bet_recommendation(
        ...
        if edge_size >= strong_threshold:
            ...
            win_rate = "52%"
            historical_context = (
                "Edge too small for profitable betting (coin flip territory)"
            )
    ...
    def generate_response(self, total_impact: float, game_details: Dict) -> str:
        """
        ...
        templates = self.response_templates.get(category, {}).get(
            "responses", [f"Impact: {total_impact:.1f} points"]
        )
        ...
        try:
            response = template.format(total_impact=total_impact, **game_details)
    ...
    def analyze_position_group_crisis(
        self, position_impacts: Dict[str, float]
    ) -> List[str]:
        """
