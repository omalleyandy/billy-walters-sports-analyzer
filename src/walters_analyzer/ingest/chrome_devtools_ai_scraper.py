...
class ChromeDevToolsAIDebugger:
    ...
    def diagnose_scraping_failure(
        self, snapshot: str, expected_elements: List[str], found_elements: List[str]
    ) -> Dict[str, Any]:
        """
...
class EnhancedChromeDevToolsOddsExtractor:
    ...
    def extract_with_ai_assistance(
        ...
        if performance_metrics:
            ai_insights["performance_analysis"] = (
                self.performance.analyze_performance_metrics(performance_metrics)
            )
            self.session_log["performance_metrics"].append(
                ai_insights["performance_analysis"]
            )
        ...
        # Source analysis
        if html_source:
            ai_insights["source_analysis"] = self.sources.analyze_page_structure(
                html_source
            )
        ...
        if not games:
            ...
            ai_insights["debugging_info"] = self.debugger.diagnose_scraping_failure(
                snapshot_text, expected_elements, found_elements
            )
        ...
        # Update success rate
        total_attempts = len(self.session_log["performance_metrics"]) + len(
            self.session_log["debugging_events"]
        )
        successful_attempts = len(
            [
                e
                for e in self.session_log["debugging_events"]
...
uid=1_78 button "O 43 -110"
    ...
    games, insights = extractor.extract_with_ai_assistance(
        sample_snapshot, performance_metrics=sample_metrics
    )

    print(f"Extracted {len(games)} games")
    print(
        f"Performance score: {insights['performance_analysis'].get('performance_score', 0)}/100"
    )
    print("\nRecommendations:")
