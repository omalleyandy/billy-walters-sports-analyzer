...
class MCPChromeDevToolsScraper:
    ...
    async def scrape_overtime_odds(
        self, sport: str = "nfl", save_ai_report: bool = True
    ) -> Dict[str, any]:
        """
        ...
        try:
            ...
            if save_ai_report:
                report_path = (
                    self.output_dir
                    / f"ai-insights-{sport}-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
                )
                save_ai_insights_report(ai_insights, report_path)

            logger.info(f"Scraping complete: {len(games)} games extracted")
            logger.info(
                f"Performance score: {ai_insights.get('performance_analysis', {}).get('performance_score', 0)}/100"
            )
            ...
            json.dump(
                {
                    ...
                    "ai_insights_summary": {
                        "extraction_success": result["ai_insights"].get(
                            "extraction_success"
                        ),
                        "performance_score": result["ai_insights"]
                        .get("performance_analysis", {})
                        .get("performance_score"),
                        "recommendations_count": len(
                            result["ai_insights"].get("recommendations", [])
                        ),
                    },
                },
                f,
...
async def main():
    ...
    parser.add_argument(
        "--output-dir", default="data/odds_chrome", help="Output directory"
    )
    parser.add_argument(
