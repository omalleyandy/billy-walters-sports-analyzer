...
def continuous_scrape(interval: int = 60, scrapers: list = None):
    ...
    print(f"\n{'='*60}")
    print(f"[*] CONTINUOUS ODDS SCRAPING")
    ...
    try:
        while True:
            ...
            print(f"   Success rate: {successes}/{successes+failures} ({100*successes/(successes+failures):.1f}%)")
...
def main():
    ...
    if args.interval < 30:
        print("[WARNING]  Warning: Intervals <30s may trigger rate limits or API quotas")
