...
class ActionNetworkCollector:
    ...
    def __init__(
        ...
        """
        self.data_dir = Path(data_dir) if data_dir else Path(__file__).parent.parent.parent.parent / 'data' / 'action_network'
        ...
        if state_file.exists():
            try:
                with open(state_file) as f:
                    state = json.load(f)
                self.last_scrape = datetime.fromisoformat(state.get('last_scrape')) if state.get('last_scrape') else None
                self.scrape_count = state.get('scrape_count', 0)
                self.error_count = state.get('error_count', 0)
                logger.info(f"Loaded state: {self.scrape_count} scrapes, last at {self.last_scrape}")
    ...
    async def _initialize_scraper(self):
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                logger.error(f"Failed to initialize scraper (attempt {attempt + 1}): {e}")
    ...
    async def run_continuous(self):
        ...
        """
        logger.info(f"Starting continuous collection (interval: {self.interval_minutes} minutes)")
            ...
            try:
                ...
                if self.last_scrape:
                    ...
                    if elapsed < self.interval_minutes:
                        ...
                        logger.info(f"Waiting {wait_minutes:.1f} minutes until next scrape...")
                ...
                if result['success']:
                    logger.info(f"Next scrape scheduled in {self.interval_minutes} minutes")
                else:
                    # On error, use shorter retry interval
                    retry_minutes = min(30, self.interval_minutes / 4)
                    logger.warning(f"Scrape failed, retrying in {retry_minutes:.0f} minutes")
...
Examples:
    ...
    parser.add_argument('--once', action='store_true', help='Run single scrape and exit')
    parser.add_argument('--continuous', action='store_true', help='Run continuous collection')
    parser.add_argument('--interval', type=int, default=240, help='Scrape interval in minutes (default: 240)')
