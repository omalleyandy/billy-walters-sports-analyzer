...
class CLVCli:
    ...
    def record_bet(self, args: argparse.Namespace) -> None:
        ...
        tracking = CLVTracking(
            ...
            if args.rec_id
            else f"manual_{datetime.now().strftime('%Y%m%d%H%M%S')}",
            game_id=game_id,
    ...
    def list_pending(self, args: argparse.Namespace) -> None:
        ...
        if pending_closing:
            ...
            for bet_id, bet in pending_closing:
                ...
                print(
                    f"  Opened: {bet.opening_line} on {bet.opening_date.strftime('%Y-%m-%d')}"
                )
                print(
                    f"  Command: python clv_track.py update-closing-line --bet-id {bet_id} --closing-line X.X"
                )
                print()
        ...
        if pending_result:
            ...
            for bet_id, bet in pending_result:
                ...
                print(
                    f"  Command: python clv_track.py update-result --bet-id {bet_id} --result [won/lost/push]"
                )
                print()
...
def main():
    ...
    record_parser.add_argument(
        "--game", required=True, help="Game ID (e.g., IND_KC or IND@KC)"
    )
    record_parser.add_argument(
        "--line", type=float, required=True, help="Opening line when bet placed"
    )
    record_parser.add_argument(
        "--amount", type=float, required=True, help="Bet amount in dollars"
    )
    record_parser.add_argument(
        "--side",
        choices=["home", "away"],
        help="Which side (auto-detected if not provided)",
    )
    record_parser.add_argument(
        "--bet-type",
        default="spread",
        choices=["spread", "total", "moneyline"],
        help="Type of bet",
    )
    record_parser.add_argument("--edge", type=float, help="Calculated edge percentage")
    record_parser.add_argument(
        "--bankroll", type=float, help="Current bankroll (default: 20000)"
    )
    record_parser.add_argument(
        "--rec-id", help="Recommendation ID (auto-generated if not provided)"
    )
    record_parser.add_argument("--notes", help="Optional notes")

    # update-closing-line command
    closing_parser = subparsers.add_parser(
        "update-closing-line", help="Update bet with closing line"
    )
    closing_parser.add_argument(
        "--bet-id", type=int, required=True, help="Bet ID to update"
    )
    closing_parser.add_argument(
        "--closing-line",
        type=float,
        required=True,
        help="Closing line right before game",
    )

    # update-result command
    result_parser = subparsers.add_parser(
        "update-result", help="Update bet with final result"
    )
    ...
    result_parser.add_argument(
        "--result",
        required=True,
        choices=["won", "lost", "push", "win", "lose", "w", "l", "p"],
        help="Bet outcome",
    )
    result_parser.add_argument(
        "--final-line", type=float, help="Final closing line if different"
    )
    result_parser.add_argument("--notes", help="Optional notes about the game")

    # summary command
    summary_parser = subparsers.add_parser(
        "summary", help="Show CLV summary statistics"
    )
    ...
    pending_parser = subparsers.add_parser(
        "list-pending", help="List bets needing updates"
    )
