...
def monitor_start(
    ...
    duration: int = typer.Option(120, "--duration", "-d", help="Duration in minutes (0 = indefinite)"),
    alert_threshold: float = typer.Option(0.5, "--threshold", "-t", help="Alert threshold in points"),
    ...
    try:
        ...
        while True:
            ...
            console.print(f"[dim][{datetime.now().strftime('%H:%M:%S')}] Check #{check_count} - {elapsed:.1f}min elapsed[/dim]")
...
def monitor_config(
    ...
    try:
        ...
        console.print(f"  Steam: {ma.steam_move_threshold} points in {ma.steam_move_window}s")
