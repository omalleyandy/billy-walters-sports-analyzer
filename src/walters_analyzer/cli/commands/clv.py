...
def clv_record(
    ...
    opening_line: float = typer.Option(
        ..., "--line", "-l", help="Opening line when bet was placed"
    ),
    stake: float = typer.Option(100.0, "--stake", "-s", help="Stake amount"),
    odds: int = typer.Option(-110, "--odds", "-o", help="Odds (American format)"),
    edge: Optional[float] = typer.Option(
        None, "--edge", "-e", help="Calculated edge %"
    ),
    ...
    console.print(
        f"[dim]walters clv update {bet_record['id']} --closing-line <LINE>[/dim]"
    )


@app.command("update")
def clv_update(
    bet_id: Optional[str] = typer.Argument(
        None, help="Bet ID to update (or 'all' for batch)"
    ),
    closing_line: Optional[float] = typer.Option(
        None, "--closing-line", "-c", help="Closing line"
    ),
    result: Optional[BetResult] = typer.Option(
        None, "--result", "-r", help="Bet result"
    ),
    auto_fetch: bool = typer.Option(
        False, "--auto", "-a", help="Auto-fetch closing lines from Overtime"
    ),
):
    """
    ...
    if not storage_path.exists():
        console.print(
            "[red]No bets found. Record a bet first with 'walters clv record'[/red]"
        )
        raise typer.Exit(1)
            ...
            if closing_line is not None:
                ...
                if bet["bet_type"] == "spread":
                    ...
                    if "over" in bet["pick"].lower():
                        bet["clv"] = (
                            closing_line - opening
                        )  # Higher close = better for over
                    else:
                        bet["clv"] = (
                            opening - closing_line
                        )  # Lower close = better for under
    ...
    with open(storage_path, "w") as f:
        if is_dict_format:
            ...
            data_to_save = {
                b.get("id", b.get("recommendation_id", "")): b for b in bets
            }
        else:
            data_to_save = bets
    ...
    for bet in bets:
        ...
        if bet_id_value == bet_id:
            ...
            clv_color = (
                "green"
                if bet.get("clv", 0) and bet["clv"] > 0
                else "red"
                if bet.get("clv", 0) and bet["clv"] < 0
                else "white"
            )
            ...
            if bet["result"] != "pending":
                ...
                console.print(
                    f"Result: [{result_color}]{bet['result'].upper()}[/{result_color}]"
                )
...
def clv_report(
    ...
    if not bets_with_clv:
        ...
        console.print(
            "[dim]Update closing lines with: walters clv update <ID> --closing-line <LINE>[/dim]"
        )
        return
    ...
    avg_clv = (
        sum(b["clv"] for b in bets_with_clv) / clv_recorded if clv_recorded > 0 else 0
    )
    ...
    # Summary panel
    clv_color = (
        "green" if clv_win_rate >= 55 else "yellow" if clv_win_rate >= 50 else "red"
    )
    ...
    if total_bets < 50:
        console.print(
            f"[yellow]⚠️ Building phase - need {50 - total_bets} more for validation[/yellow]"
        )
    elif total_bets < 100:
        console.print(
            f"[yellow]⚠️ Validation phase - need {100 - total_bets} more for statistical proof[/yellow]"
        )
    else:
    ...
    console.print(
        f"  CLV Win Rate: [{clv_color}]{clv_win_rate:.1f}%[/{clv_color}] (target: >55%)"
    )
    console.print(
        f"  Average CLV: [{avg_clv_color}]{avg_clv:+.2f}[/{avg_clv_color}] (target: >+0.3)"
    )
    ...
    if detailed:
        ...
        for bet in sorted(bets, key=lambda x: x.get("timestamp", ""), reverse=True):
            ...
            result_style = (
                "green" if result == "win" else "red" if result == "loss" else "yellow"
            )
...
@app.command("list")
def clv_list(
    status: Optional[BetResult] = typer.Option(
    ...
    if not storage_path.exists():
        console.print("[yellow]No bets found. Record some bets first![/yellow]")
        console.print(
            '[dim]walters clv record "DET @ CHI" "DET -3.5" -3.5 --stake 200[/dim]'
        )
        return
    ...
    bets = sorted(
        bets, key=lambda x: x.get("timestamp", x.get("opening_date", "")), reverse=True
    )[:limit]
...
def clv_analyze(
    by: str = typer.Option(
        "week", "--by", "-b", help="Group by: week, sport, bet_type, edge"
    ),
):
    """
