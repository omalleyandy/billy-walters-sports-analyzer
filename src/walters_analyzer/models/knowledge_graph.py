...
class BettingKnowledgeGraph:
    ...
    def __init__(self, storage_path: Optional[Path] = None):
        ...
        self.game_evaluations: Dict[str, Set[str]] = defaultdict(
            set
        )  # game_id -> evaluation_ids
        self.evaluation_recommendations: Dict[str, Set[str]] = defaultdict(
            set
        )  # eval_id -> rec_ids

        # Performance tracking
        self.bet_outcomes: Dict[
            str, Dict[str, Any]
    ...
    def add_recommendation(
        ...
        if evaluation_id:
            self.evaluation_recommendations[evaluation_id].add(
    ...
    def get_performance_summary(
        ...
        wins = sum(
            1
            for r in recs
            if self.bet_outcomes.get(r.recommendation_id, {}).get("result") == "win"
        )
        losses = sum(
            1
            for r in recs
            if self.bet_outcomes.get(r.recommendation_id, {}).get("result") == "loss"
        )
        pushes = sum(
            1
            for r in recs
            if self.bet_outcomes.get(r.recommendation_id, {}).get("result") == "push"
        )
        ...
        return {
            ...
            if total_bets > 0
            else 0,  # Assuming $100 unit
            "avg_clv": avg_clv,
            "clv_beat_rate": sum(1 for v in clv_values if v > 0) / len(clv_values)
            if clv_values
            else 0,
    ...
    def find_similar_matchups(
        ...
        for other_game_id, other_game in self.games.items():
            ...
            if not (
                ...
                in (other_game.home_team_id, other_game.away_team_id)
                or target_game.away_team_id
                in (other_game.home_team_id, other_game.away_team_id)
            ):
                continue
    ...
    def save(self, path: Optional[Path] = None) -> None:
        ...
        data = {
            ...
            "recommendations": {
                rid: r.model_dump() for rid, r in self.recommendations.items()
            },
            "team_games": {tid: list(gids) for tid, gids in self.team_games.items()},
            "game_evaluations": {
                gid: list(eids) for gid, eids in self.game_evaluations.items()
            },
    ...
    def load(self, path: Optional[Path] = None) -> None:
        ...
        for gid, gdata in data.get("games", {}).items():
            ...
            if "kickoff_datetime" in gdata:
                gdata["kickoff_datetime"] = datetime.fromisoformat(
                    gdata["kickoff_datetime"]
                )
        ...
        # Load relationships
        self.team_games = {
            tid: set(gids) for tid, gids in data.get("team_games", {}).items()
        }
        self.game_evaluations = {
            gid: set(eids) for gid, eids in data.get("game_evaluations", {}).items()
        }
        self.evaluation_recommendations = {
            eid: set(rids)
            for eid, rids in data.get("evaluation_recommendations", {}).items()
        }
