...
class TeamContext(BaseModel):
    ...
    power_rating: float = Field(
        ge=-10.0, 
        le=10.0,
        description="Billy Walters power rating (-10 to +10)"
    )
    power_rating_rank: Optional[int] = Field(
        default=None,
        ge=1,
        le=32,
        description="Rank among all teams (1=best)"
    )
    ...
    conference_record: Optional[Record] = Field(default=None, description="Conference record")
    
    # Performance Metrics
    points_per_game: Optional[float] = Field(default=None, ge=0, description="Average PPG")
    points_allowed_per_game: Optional[float] = Field(default=None, ge=0, description="Average PA/G")
    point_differential: Optional[float] = Field(default=None, description="Point differential")
    ...
    strength_of_schedule: Optional[float] = Field(
        ...
        ge=-10.0,
        le=10.0,
        description="Average opponent power rating"
    )
...
class ScheduleHistory(BaseModel):
    ...
    rest_advantage_vs_opponent: Optional[int] = Field(
        default=None,
        description="Extra rest days vs opponent (can be negative)"
    )
    
    # Travel Analysis
    previous_game_location: Optional[str] = Field(default=None, description="City of last game")
    travel_distance_miles: Optional[float] = Field(
        default=None,
        ge=0,
        description="Miles traveled since last game"
    )
    time_zones_crossed: int = Field(
        default=0,
        ge=0,
        le=3,
        description="Time zones crossed (0-3)"
    )
    consecutive_away_games: int = Field(
        default=0,
        ge=0,
        description="Number of consecutive away games"
    )
    
    # Schedule Density
    games_in_last_14_days: int = Field(default=0, ge=0, le=3, description="Recent game density")
    games_in_last_21_days: int = Field(default=0, ge=0, le=4, description="3-week density")
    ...
    def has_travel_disadvantage(self) -> bool:
        ...
        return (
            (self.travel_distance_miles or 0) > 1500 or
            self.time_zones_crossed >= 2 or
            self.consecutive_away_games >= 3
        )
...
class GameContext(BaseModel):
    ...
    playoff_implications: Optional[str] = Field(
        default=None,
        description="Description of playoff stakes"
    )
    home_team_playoff_probability: Optional[float] = Field(
        default=None,
        ge=0.0,
        le=1.0,
        description="Home team playoff probability"
    )
    away_team_playoff_probability: Optional[float] = Field(
        default=None,
        ge=0.0,
        le=1.0,
        description="Away team playoff probability"
    )
    
    # Situational Factors
    is_primetime: bool = Field(default=False, description="Thursday/Sunday/Monday night")
    ...
    # Revenge/Lookahead
    is_revenge_game: Optional[bool] = Field(default=None, description="Rematch from last season")
    has_lookahead_spot: Optional[bool] = Field(
        default=None,
        description="Team looking ahead to next opponent"
    )
    
    # Recent Results (for bounce-back)
    home_team_previous_margin: Optional[int] = Field(
        default=None,
        description="Home team's margin in last game (negative = loss)"
    )
    away_team_previous_margin: Optional[int] = Field(
        default=None,
        description="Away team's margin in last game (negative = loss)"
    )
...
class WeatherContext(BaseModel):
    ...
    feels_like_f: Optional[int] = Field(default=None, description="Feels-like temperature")
...
class SFactorGamePackage(BaseModel):
    ...
    # Weather Context
    weather: Optional[WeatherContext] = Field(default=None, description="Weather forecast")
    ...
    data_sources: Dict[str, str] = Field(default_factory=dict, description="Source tracking")
...
if __name__ == "__main__":
    ...
    print(f"  Travel: {schedule.travel_distance_miles} miles, {schedule.time_zones_crossed} zones")
    ...
    print(f"  Game: {game.away_team_context.team_name} @ {game.home_team_context.team_name}")
    print(f"  Matchup: {game.quality_matchup_description}")
    print(f"  Power Rating Diff: {game.power_rating_differential:+.1f} (home advantage)")
