...
def test_knowledge_graph():
    ...
    print("✓ Added power rating snapshots")
    ...
    if evaluation.edge_percent >= 5.5:
        ...
        print(
            f"ℹ No bet recommended (edge {evaluation.edge_percent:.1f}% < 5.5% minimum)"
        )
...
def main():
    ...
    try:
        ...
        if all([schema_ok, graph_ok, compliance_ok]):
            ...
            print(
                "- Run: python -m walters_analyzer.cli.knowledge_graph_cli test-recommendation"
            )
            print(
                "- Run: python -m walters_analyzer.cli.knowledge_graph_cli validate-all"
            )
