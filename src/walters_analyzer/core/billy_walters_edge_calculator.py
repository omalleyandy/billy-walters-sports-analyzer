...
class BillyWaltersEdgeCalculator:
    ...
    def calculate_complete_edge(
        ...
        if base_edge_points < 1.0 and total_edge_pct >= 5.5:
            warnings.append(
                "[WARNING] Edge primarily from S-factors - verify calculations"
            )
    ...
    def _determine_confidence(self, edge_pct: float, crossed_numbers: List[int]) -> str:
...
if __name__ == "__main__":
    ...
    result = calc.calculate_complete_edge(
        our_line=-3.0, market_line=-4.0, sfactor_points=0
    )
