...
class SessionManager:
    ...
    def load_latest_session(self, week: Optional[int] = None) -> Optional[SessionContext]:
        """
        ...
        for session_file in sessions:
            ...
            if week is None or session.week == week:
                logger.info(
                    ...
                    extra={"week": session.week, "age_hours": (datetime.now() - session.last_updated).total_seconds() / 3600}
                )
                return session
