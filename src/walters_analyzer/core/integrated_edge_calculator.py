"""
Integrated Edge Calculator with Sharp Money Signals & News/Injury E-Factors

Combines Billy Walters methodology with multiple data sources:
...
5. News/injury E-Factor adjustments (coaching, injuries, transactions)
...
# Import news/injury E-Factor aggregator (lazy load to avoid circular deps)
try:
...
class IntegratedEdgeAnalysis:
    """Complete edge analysis combining power ratings, sharp money, and E-Factors."""
    ...
    # E-Factor components (news/injury)
    efactor_adjustment: float = 0.0  # Adjustment from E-Factors
    efactor_sources: List[str] = field(default_factory=list)
    efactor_details: Dict = field(default_factory=dict)
...
class IntegratedEdgeCalculator:
    ...
    def _calculate_efactor_adjustment(
        self, away_team: str, home_team: str, league: str = "nfl"
    ) -> tuple[float, List[str], Dict]:
        """
        Calculate E-Factor adjustment from news/injury data.
        ...
        Returns:
            Tuple of (adjustment_points, source_list, details_dict)
        """
        if not self.efactor_aggregator:
            return 0.0, [], {}

        # Placeholder: In async context, would call aggregator
        # For now, return structure that integrate_game method expects
        # In production, this would be called from async analyze_game_async
        return 0.0, [], {}
    ...
    def _calculate_key_number_premium(
        self, our_line: float, market_line: float
    ...
    def _get_confidence_level(
        self, edge_pct: float, crossed_numbers: list, sharp_alignment: str
    ) -> str:
        ...
        if sharp_alignment == "CONFIRMS" and base_confidence != "HIGH":
            base_confidence = {"LOW": "MEDIUM", "MEDIUM": "HIGH"}.get(
                base_confidence, base_confidence
            )
        elif sharp_alignment == "CONTRADICTS":
            base_confidence = {"HIGH": "MEDIUM", "MEDIUM": "LOW"}.get(
                base_confidence, base_confidence
            )
    ...
    def analyze_game(
        ...
        if sharp_signal:
            ...
            if sharp_signal.is_sharp_side:
                ...
                notes.append(
                    f"Sharp money CONFIRMS our pick ({sharp_signal.divergence:+d} divergence)"
                )
                ...
                warnings.append(
                    f"Sharp money CONTRADICTS our pick ({sharp_signal.divergence:+d} divergence)"
                )

            else:
                sharp_alignment = "NEUTRAL"
                notes.append(
                    f"Sharp money neutral ({sharp_signal.divergence:+d} divergence)"
                )
            ...
            if sharp_signal.line_movement:
                direction = (
                    "toward"
                    if (
                        (sharp_signal.line_movement < 0 and pick_side == "home")
                        or (sharp_signal.line_movement > 0 and pick_side == "away")
                    )
                    else "against"
                )
                notes.append(
                    f"Line moved {abs(sharp_signal.line_movement):.1f} pts "
                    f"{direction} our pick"
                )
        ...
        if base_edge_points > 7.0 and sharp_alignment != "CONFIRMS":
            warnings.append("Large edge without sharp confirmation - value trap")
        ...
        return IntegratedEdgeAnalysis(
            ...
            efactor_details=efactor_details,
    ...
    def print_analysis(self, analysis: IntegratedEdgeAnalysis) -> None:
        ...
        print(
            f"  Key Numbers: {analysis.crossed_key_numbers} "
            f"(+{analysis.key_number_premium_pct:.1f}%)"
        )
        ...
        if analysis.efactor_adjustment != 0.0:
            print(f"\n📰 E-FACTOR (News/Injury):")
            print(f"  Adjustment: {analysis.efactor_adjustment:+.1f} pts")
            if analysis.efactor_sources:
                print(f"  Sources: {', '.join(analysis.efactor_sources)}")
...
def main():
    ...
    if data_path.exists():
        ...
        if alt_path.exists():
            ...
            print(
                "⚠️ No Action Network data found - running without sharp money signals"
            )
