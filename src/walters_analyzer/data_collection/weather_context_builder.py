...
class WFactor(BaseModel):
    ...
    impact_points: float = Field(description="Weather factor points (converted to spread via 5:1 ratio)")
    impact_to_spread: float = Field(description="Direct spread point impact (impact_points / 5)")
    affects: str = Field(description="What it affects: 'total' or 'spread' or 'both'")
    direction: str = Field(description="Direction: 'favors_home', 'favors_away', 'neutral'")
    confidence: float = Field(ge=0.0, le=1.0, description="Confidence in this factor (0-1)")
...
class WeatherContextBuilder:
    ...
    def _calculate_precipitation_factors(
        ...
        # SNOW
            ...
            snow_impact = -20 if "HEAVY" in weather.precipitation_type.value.upper() else -15
            factors.append(
                WFactor(
    ...
    def _assess_warm_team_suitability(
        ...
        if temp > 70 and wind <= 10 and weather.precipitation_type == Precipitation.NONE:
    ...
    def _classify_wind_condition(self, wind_speed: Optional[int]) -> Optional[WindCondition]:
