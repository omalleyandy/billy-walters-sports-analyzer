...
class GameOdds:
    ...
    def spread_sharp_side(self) -> Optional[str]:
        ...
        if self.consensus_spread and self.consensus_spread.betting.is_public_side:
            return self.consensus_spread_away.team_abbr if self.consensus_spread_away else "AWAY"
        if self.consensus_spread_away and self.consensus_spread_away.betting.is_sharp_side:
...
class ActionNetworkParser:
    ...
    def _parse_markets(self, game: GameOdds, markets: dict, 
                       away_team: TeamInfo, home_team: TeamInfo) -> None:
        ...
        if consensus:
            ...
            self._parse_book_odds(game, inner, away_team, home_team, 
                                  is_consensus=True, book_name="Consensus")
        ...
        for book_id, book_data in markets.items():
            ...
            self._parse_book_odds(game, inner, away_team, home_team,
                                  is_consensus=False, book_name=book_name, 
                                  book_id=int(book_id))
    
    def _parse_book_odds(self, game: GameOdds, odds_data: dict,
                         away_team: TeamInfo, home_team: TeamInfo,
                         is_consensus: bool, book_name: str,
    ...
    def _parse_odds_line(self, line_data: dict, line_type: str,
                         away_team: TeamInfo, home_team: TeamInfo) -> OddsLine:
    ...
    def get_best_lines(self, game: GameOdds, line_type: str = 'spread',
                       side: str = 'home') -> list[tuple[str, float, int]]:
        """
    ...
    def to_summary_dict(self) -> dict:
        ...
        return {
            'league': self.league,
            'timestamp': self.data_timestamp.isoformat() if self.data_timestamp else None,
            ...
            'games': [
                {
                    ...
                    'spread_home_tix_pct': g.consensus_spread.betting.tickets_percent if g.consensus_spread else None,
                    'spread_home_money_pct': g.consensus_spread.betting.money_percent if g.consensus_spread else None,
                    'spread_away_tix_pct': g.consensus_spread_away.betting.tickets_percent if g.consensus_spread_away else None,
                    'spread_away_money_pct': g.consensus_spread_away.betting.money_percent if g.consensus_spread_away else None,
...
def main():
    ...
    for game in games:
        ...
        if game.consensus_spread:
            ...
            print(f"    Home: {s.betting.tickets_percent}% tix / {s.betting.money_percent}% $")
            if sa:
                print(f"    Away: {sa.betting.tickets_percent}% tix / {sa.betting.money_percent}% $")
