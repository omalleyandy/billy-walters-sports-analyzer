...
def classify_recent_performance(
    last_5_record: Record, current_streak: Optional[str] = None
) -> str:
    """
...
def calculate_schedule_difficulty(opponents_power_ratings: List[float]) -> float:
    """
...
class TeamContextBuilder:
    ...
    def validate_context(self, context: TeamContext) -> Tuple[bool, List[str]]:
        ...
        if context.points_allowed_per_game is not None:
            if (
                context.points_allowed_per_game < 0
                or context.points_allowed_per_game > 60
            ):
    ...
    def _estimate_power_rank(
        self, power_rating: float, all_ratings: List[float]
    ) -> int:
        """
...
if __name__ == "__main__":
    ...
    contexts = builder.build_contexts_from_massey_and_espn(massey_sample, espn_sample)
