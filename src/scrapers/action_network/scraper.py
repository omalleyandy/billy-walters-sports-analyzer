...
class ActionNetworkScraper:
    ...
    DIVERGENCE_THRESHOLDS = {
        ...
        "nba": {"moderate": 5, "strong": 10, "very_strong": 15},
        "ncaab": {
    ...
    def get_min_divergence(cls, league: str) -> int:
        ...
        thresholds = cls.DIVERGENCE_THRESHOLDS.get(
            league.lower(), cls.DIVERGENCE_THRESHOLDS["nfl"]
        )
        ...
        try:
            ...
            if cf_check > 0:
                ...
                await self.page.wait_for_selector(
                    'text="Checking your browser"', state="hidden", timeout=30000
                )
    ...
    async def _extract_next_data(self) -> Optional[dict]:
        ...
        try:
            ...
            await self.page.wait_for_selector(
                "script#__NEXT_DATA__", state="attached", timeout=10000
            )
    ...
    def _parse_game_data(self, game_data: dict, all_books: dict) -> Optional[GameOdds]:
        ...
        try:
            ...
            away_record = (
                f"{away_standings.get('win', 0)}-{away_standings.get('loss', 0)}"
            )
            home_record = (
                f"{home_standings.get('win', 0)}-{home_standings.get('loss', 0)}"
            )
        ...
        try:
            ...
            if not data:
                ...
                await self.page.screenshot(
                    path=str(self.data_dir / "error_screenshot.png")
                )
                return games
    ...
    def get_sharp_plays(
        ...
        games: list[GameOdds],
        league: str = "nfl",
        min_divergence: Optional[int] = None,
    ) -> list[dict]:
        """
    ...
    def save_data(self, games: list[GameOdds], league: str = "nfl") -> Path:
        ...
        output["divergence_thresholds"] = self.DIVERGENCE_THRESHOLDS.get(
            league, self.DIVERGENCE_THRESHOLDS["nfl"]
        )

        # Save to file
        filename = (
            f"{league}_odds_week{week}_{timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        if games:
            ...
            if sharp_plays:
                ...
                for play in sharp_plays:
                    ...
                    print(
                        f"  Tickets: {play['tickets_pct']}% | Money: {play['money_pct']}%"
                    )
            ...
            for game in games:
                spread = (
                    f"{game.home_team} {game.spread_line:+.1f}"
                    if game.spread_line
                    else "N/A"
                )
                sharp = (
                    f" ⭐ Sharp: {game.sharp_spread_side}"
                    if game.sharp_spread_side
                    else ""
                )
