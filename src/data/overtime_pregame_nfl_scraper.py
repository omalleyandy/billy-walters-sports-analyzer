...
Scrapes NFL pre-game betting lines (spreads, totals, moneylines) from Overtime.ag
using Playwright browser automation. Supports multiple betting periods (Game, 1st Half, 1st Quarter).
...
class OvertimeNFLScraper:
    ...
    def __init__(
        ...
        Args:
            ...
            output_dir: Directory to save scraped data (default: output/overtime/nfl/pregame)
    ...
    async def scrape(self) -> Dict[str, Any]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Enhanced debug: Check what's on the page with better diagnostics
                debug_info = await page.evaluate("""
                    () => {
                        ...
                        return {
                            ...
                            nflSectionVisible: bodyText.includes('NFL-Game') || bodyText.includes('NFL')
                        };
                    }
                """)
                ...
                if debug_info.get("buttonCount", 0) == 0:
                    ...
                    print("   -> Lines post after Monday Night Football ends")
    ...
    async def _navigate_to_nfl(self, page: Page) -> None:
        ...
        try:
            # Use JavaScript to click NFL-Game/1H/2H/Qrts
            await page.evaluate("""
                () => {
            ...
            games_data = await page.evaluate(
                ...
                (period) => {
                    ...
                    if (teams.length >= 2) {
                        ...
                        // Format: [visitor_spread, visitor_total, home_spread, home_total]
                        // OR: [visitor_spread, visitor_ml, visitor_total, home_spread, home_ml, home_total]
                        if (bettingLines.length >= 4) {
                            // Check if moneylines are included (more than 4 buttons)
                            if (bettingLines.length === 4) {
