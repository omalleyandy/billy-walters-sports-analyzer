...
class NetworkAnalyzer:
            ...
            rate_limits.append(
                {
                    "type": "high_frequency",
                    "domains": sorted(
                        high_frequency_domains, key=lambda x: x["count"], reverse=True
                    )[:10],
                }
            )
    ...
    def find_bottlenecks(self) -> List[Dict[str, Any]]:
        ...
        # Large responses (>1MB)
        large_responses = [
            r for r in self.responses if r.get("size_bytes", 0) > 1_000_000
        ]
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"Total Bytes: {self.network_metrics.get('total_bytes', 0) / 1024 / 1024:.2f} MB"
        )
