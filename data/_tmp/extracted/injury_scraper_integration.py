...
from billy_walters_injury_valuation import (
    PlayerValue, InjuryStatus, PlayerPosition, 
    InjuryImpact, TeamInjuryAnalyzer
)
...
class InjuryDataEnricher:
    ...
    def parse_position(self, position_text: str) -> PlayerPosition:
        ...
        position_map = {
            ...
            "OL": PlayerPosition.OL, "OT": PlayerPosition.OL, "OG": PlayerPosition.OL, 
            "C": PlayerPosition.OL, "G": PlayerPosition.OL, "T": PlayerPosition.OL,
            "DL": PlayerPosition.DL, "DE": PlayerPosition.DL, "DT": PlayerPosition.DL,
            "NT": PlayerPosition.DL, "EDGE": PlayerPosition.DL,
            "LB": PlayerPosition.LB, "ILB": PlayerPosition.LB, "OLB": PlayerPosition.LB,
            "MLB": PlayerPosition.LB,
            "CB": PlayerPosition.CB, "DB": PlayerPosition.CB,
            "S": PlayerPosition.S, "FS": PlayerPosition.S, "SS": PlayerPosition.S,
            "K": PlayerPosition.K,
            "P": PlayerPosition.P
        }
    ...
    def estimate_player_value(self, player_name: str, position: PlayerPosition, 
                            team: str, snap_pct: float = 0) -> float:
        """
    ...
    def analyze_game_injuries(self, home_injuries: List[Dict], 
        ...
        recommendation = self._generate_recommendation(net_impact, home_impact, away_impact)
        ...
        return {
            ...
            "key_matchup_advantages": self._identify_key_advantages(home_impact, away_impact)
        }
    
    def _generate_recommendation(self, net_impact: float, 
                                home_impact: Dict, away_impact: Dict) -> str:
    ...
    def _identify_key_advantages(self, home_impact: Dict, away_impact: Dict) -> List[str]:
        ...
        if abs(home_pass - away_pass_d) > 2:
            if home_pass > away_pass_d:
                advantages.append("Away passing attack advantage due to home secondary injuries")
            else:
                advantages.append("Home passing attack advantage due to away secondary injuries")
        ...
        if abs(home_run - away_run_d) > 2:
            if home_run > away_run_d:
                advantages.append("Away ground game advantage due to home front-7 injuries")
            else:
                advantages.append("Home ground game advantage due to away front-7 injuries")
        
        return advantages if advantages else ["No significant unit mismatches identified"]
...
def demonstrate_enhanced_analysis():
    ...
    for injury in home['key_injuries']:
        print(f"    - {injury['player']} ({injury['position']}): {injury['impact']:+.1f} pts")
