...
class ScrapedPlayerData:
    ...
    def to_valuation_format(self) -> Dict:
        ...
        if self.stats:
            player_value = InjuryDataValidator.estimate_player_value_from_stats(self.stats)
        else:
...
class InjuryDataProcessor:
    ...
    def _analyze_game(self, game_id: str, game_data: pd.DataFrame) -> Dict:
        ...
        home_team = game_data[game_data['is_home'] == True] if 'is_home' in game_data else game_data.iloc[:len(game_data)//2]
        away_team = game_data[game_data['is_home'] == False] if 'is_home' in game_data else game_data.iloc[len(game_data)//2:]
        ...
        for _, player in home_team.iterrows():
            scraped = ScrapedPlayerData(
                ...
                last_update=player.get('last_update', datetime.now().strftime("%Y-%m-%d")),
    ...
    def _generate_specific_insights(self, analysis: Dict) -> List[str]:
        ...
        home_ol_injuries = [i for i in analysis['home_team_analysis']['moderate_injuries'] 
        ...
        return insights if insights else ["✓ Injury impact within normal variance (< 1 point)"]
    ...
    def _generate_insights(self, games: List[Dict]) -> List[str]:
        ...
        avg_home_advantage = sum(home_advantages) / len(home_advantages) if home_advantages else 0
...
def integrate_with_scraper(scraper_output_file: str):
    ...
    print("="*60)
    print(f"✓ Analyzed {analysis['total_players_analyzed']} players across {len(analysis['games'])} games")
    print(f"✓ Found {len(analysis['betting_opportunities'])} high-confidence opportunities")
    ...
    if analysis['betting_opportunities']:
        ...
        for i, opp in enumerate(analysis['betting_opportunities'][:3], 1):
            print(f"{i}. {opp['game']}: {opp['edge']:.1f} point edge ({opp['expected_roi']*100:.0f}% ROI)")
