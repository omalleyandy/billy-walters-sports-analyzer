...
class BillyWaltersValuationSystem:
    ...
    INJURY_IMPACTS = {
        InjuryType.CONCUSSION: (0.85, 7, 0.92),        # 85% if plays, 7 day recovery, 92% for 2 weeks after
    ...
    @classmethod
    def calculate_player_value(cls, 
                              position: PlayerPosition,
                              metrics: PlayerMetrics,
                              is_nfl: bool = True) -> float:
        """
    ...
    @classmethod
    def calculate_injury_adjusted_value(cls,
                                       player_value: float,
                                       injury_type: Optional[InjuryType],
                                       days_since_injury: int = 0) -> Tuple[float, str]:
        """
        ...
        if days_since_injury < recovery_days:
            ...
            current_multiplier = immediate_impact + (1.0 - immediate_impact) * recovery_progress
            ...
            if weeks_post_recovery < 2:
                current_multiplier = lingering_effect + (1.0 - lingering_effect) * (weeks_post_recovery / 2)
                adjusted_value = player_value * current_multiplier
                explanation = (
    ...
    @classmethod
    def calculate_team_injury_impact(cls,
                                    injured_players: List[Dict]) -> Dict:
        """
        ...
        if total_impact >= 7.0:
            assessment = "SEVERE IMPACT: Line should move 7+ points. Strong fade candidate."
            confidence = "HIGH"
        elif total_impact >= 4.0:
            assessment = "MAJOR IMPACT: Line should move 4-7 points. Clear disadvantage."
            confidence = "HIGH"
        elif total_impact >= 2.0:
            assessment = "MODERATE IMPACT: Line should move 2-4 points. Notable disadvantage."
            confidence = "MEDIUM"
        elif total_impact >= 1.0:
            assessment = "MINOR IMPACT: Line should move 1-2 points. Slight disadvantage."
            confidence = "MEDIUM"
        else:
            assessment = "MINIMAL IMPACT: Line movement under 1 point. Negligible effect."
            confidence = "LOW"
        ...
        return {
            ...
            'recommended_line_adjustment': round(total_impact * 0.85, 1),  # Market typically underreacts by 15%
            'betting_recommendation': cls._generate_betting_recommendation(total_impact)
        }
    ...
    def _generate_betting_recommendation(total_impact: float) -> str:
        ...
        if total_impact >= 5.0:
            ...
            return "MODERATE PLAY: Consider fading injured team if line moves < 3 points."
        elif total_impact >= 1.5:
            return "SELECTIVE PLAY: Only bet if you have additional edges beyond injuries."
        else:
...
def analyze_game_injuries(home_injuries: List[Dict], away_injuries: List[Dict]) -> Dict:
    ...
    net_impact = away_analysis['total_impact_points'] - home_analysis['total_impact_points']
    ...
    return {
        ...
        'suggested_line_move': round(net_impact * 0.85, 1),  # Market underreaction factor
        'confidence_level': 'HIGH' if abs(net_impact) >= 3.0 else 'MEDIUM' if abs(net_impact) >= 1.5 else 'LOW'
    }
...
if __name__ == "__main__":
    ...
    print(f"Away Team Impact: -{game_analysis['away_team_analysis']['total_impact_points']} points")
