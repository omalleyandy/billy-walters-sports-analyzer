...
class PlayerValue:
    ...
    def calculate_actual_value(self) -> float:
        ...
        snap_adjustment = (
            self.snap_percentage / 100.0 if self.snap_percentage > 0 else 1.0
        )
...
class InjuryImpact:
    ...
    def calculate_total_impact(self) -> Dict[str, float]:
        ...
        total_impact = (
            direct_loss + scheme_adjustment + cohesion_adjustment
        ) * position_mult
        ...
        return {
            ...
            "recommendation": self._get_betting_recommendation(
...
class TeamInjuryAnalyzer:
    ...
    def calculate_cumulative_impact(self) -> Dict[str, any]:
        ...
        total_impact = sum(
            inj.calculate_total_impact()["point_spread_adjustment"]
        ...
        for group_name, positions in position_groups.items():
            group_injuries = [
                inj for inj in self.injuries if inj.player.position in positions
            ]
        ...
        if abs(adjusted_impact) < 3:
            ...
            recommendation = (
                f"SEVERE injury situation ({adjusted_impact:+.1f} pts) - STRONG FADE"
            )
        else:
            recommendation = (
                f"CATASTROPHIC injuries ({adjusted_impact:+.1f} pts) - MAX BET AGAINST"
            )
...
def demonstrate_injury_analysis():
    ...
    for i, (name, value) in enumerate(
        [("Joe Thuney", 3.5), ("Creed Humphrey", 4.0), ("Trey Smith", 3.0)]
    ):
        player = PlayerValue(
            name=name,
