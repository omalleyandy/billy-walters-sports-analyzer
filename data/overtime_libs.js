...
(3860,15-16): run-time error JS1010: Expected identifier: .
(3860,15-16): run-time error JS1195: Expected expression: .
 */
/*
 * Date Format 1.2.3
...
/*
* revisar localizacion de este js
* */
var wrapper = $("#wrapper")
if(wrapper.hasClass("active")){
    (function() {
        var dd = document.getElementsByClassName('sidebar-wrapper')[0];
        var prevScrollpos = dd.scrollTop;
        dd.onscroll = function () {
            var currentScrollPos = dd.scrollTop;
            if (prevScrollpos > currentScrollPos) {
                $(dd).find(".logo").slideDown('slow');
//          dd.style.top = "0";

            } else {
//          dd.style.top = "-75px";
                $(dd).find(".logo").slideUp('slow')
            }
...
UI.Alert = function (msg, title, type = 'error', msgIsHTML = false) {
    if (title == null) title = "";

    Swal.fire({
        title: title,
        text: msgIsHTML ? null : msg,
        html: msgIsHTML ? msg : null,
        icon: type,
...
UI.AlertCustomerMessageAndForget = function (msg, title, forget, type = 'error') {
    if (title == null) title = "";
...
UI.AlertCustomerMessage = function (msg, title, type = 'error') {
    if (title == null) title = "";
    ...
    Swal.fire({
        ...
        confirmButtonText: 'Close',
        customClass:{
            title: 'alertCustomerCommentsTitle',
            content:'alertCustomerCommentsBody'
        }
    })

};

UI.AlertBootstrap = function (msg, title, fcnHide) {
    if (title == null) title = "";
...
UI.ConfirmBootstrap = function (msg, fcnOk, fcnCancel, title) {
    if (!UI.confirmationShown) {
        if (title == null) title = "Confirmation";
...
UI.ScrollDown = function (sectionId, element) {
  setTimeout(function () {
    var anchor = $('#' + element);
    $('#' + sectionId).animate({
      scrollTop: anchor.offset() ? $('#' + sectionId).scrollTop() + anchor.offset().top - 100 : 99999
    }, 'slow');
  }, 100);
};
...
UI.Notify = function (msg, from, align, offset, type, title, autoHide = true, id = '') {
    if (!type) type = "info";
  $.notify({
    message: msg,
    title: title,
    ...
    callback: function (id) {
      if (id == 'closeAll') $.notifyClose('all');
    }
    }, {
        placement: {
            from: from,
        ...
        offset: offset,
    type: type,
    timer: autoHide ? 10000 : null,
    allow_dismiss: autoHide ? null : true,
    });
};
...
;
function ServiceCaller($http, handler, method, data) {
  ...
  var t = getCookie("t");
  if (t)
    this._$http.defaults.headers.common.Authorization = 'Bearer ' + t;

  //Document_Function
  this.ValidateSession = function (response) {
    var valid = true;
    ...
    if (!valid) {
      ...
      if (SETTINGS.LoginUrl)
        document.location.assign(SETTINGS.LoginUrl);
    }
  };
  ...
  this.GetActiveSessionId = function () {
    ...
    return this._$http({
      ...
      url: appModule.Root + '/Api/cookieless.aspx'
    }).success(function (response) {
      if (response && response.sid) localStorage["sid"] = response.sid;
      //console.log(localStorage["sid"]);
      return response.sid;
    });
  };

  this.GetStoredActiveSessionId = function () {
    if (!COOKIELESS) return null;
    //console.log(localStorage["sid"]);
    return '/(S(' + localStorage["sid"] + "))";
  };

  //Document_Function
  this.POST = function (datap, methodp, handlerp) {
    ...
    if (offlineEnable) {
      var offlineItem = methodp + "_" + (datap ? Object.values(datap).join("_") : "").RemoveSpecials();

      if (localStorage[offlineItem]) {
        return new Promise(function (resolve) {
          var data = JSON.parse(localStorage[offlineItem]);
          resolve(data);
        }).then();
      }
    }

    var sid = this.GetStoredActiveSessionId();
    var url = (sid && COOKIELESS ? sid : "") + appModule.Root + '/Api/' + this._handler + '.asmx/' + this._method;
    var _self = this;
    ...
    return this._$http({
      ...
      if (offlineEnable && response && response.d && response.d.Data)
        localStorage[offlineItem] = JSON.stringify(response.d.Data);
      if (parent.ValidateSession(response, appModule.Root + '/Api/' + handlerp + '.asmx/' + methodp)) return response.d.Data;
      else return null;
    }).error(function (err) {
      _self.GetActiveSessionId();
    }).then(function (result) {
      if ($.type(result) === "string") return result;
      var ret = (result && result.data && result.data.d) ? (result.data.d.Data || result.data.d || {}) : {};
      if (result.data && result.data.d) {
        ret.ResultCode = result.data.d.Code || 0;
        ret.ResultMessage = result.data.d.Message || (ret.ResultMessage || null);
      }
      return ret;
    });
  };
...
var ClientCustom = ClientCustom || {}

//JQuery ready
ClientCustom.OnReady = function () {

};

//After App Controller Init Completed
ClientCustom.Init = function (scope, translatorService) {
  //ClientCustom._showCookiesDisclaimer(null, translatorService);
  scope.ShowAvailableBalance = true;
};

//Before customer logout
ClientCustom.OnLogOut = function ($scope) {
...
//5D
ClientCustom._showCookiesDisclaimer = function (callback, translatorService) {
  ...
  function addSessionData(id, item) {
    window.localStorage.setItem(id, JSON.stringify(item));
  };
  let cookiesDisclaimer = getSessionData('cookiesDisclaimer') ? getSessionData('cookiesDisclaimer') : null;
  if (!cookiesDisclaimer) {
    Swal.fire({
      title: translatorService.Translate('How we use cookies'),
      allowOutsideClick: false,
      text: `${translatorService.Translate('We use cookies to enhance your gaming and browsing experience while visiting our website. They help in providing us traffic analysis, fraud prevention and to comply with our legal and regulatory obligations. To learn more about how we use information obtained from cookies click on our Privacy Policy.')}`,
      showConfirmButton: true,
      showCancelButton: true,
      focusConfirm: true,
      confirmButtonText:
        `<span style="color: black">${translatorService.Translate('Accept')}</span>`,
      cancelButtonText:
        `<span style="color: black">${translatorService.Translate('Privacy Policy')}</span>`,
      confirmButtonColor: '#FFD87F',
      cancelButtonColor: '#EDF0F2',
    }).then((result) => {
      if (result.isConfirmed) {
        addSessionData('cookiesDisclaimer', true);
        if (callback) callback();
      }
      else if (result.dismiss === "cancel") {
        Swal.fire({
          title: translatorService.Translate('Privacy Policy'),
          html: `<div style="width: 290px; max-height: 400px; overflow-y: scroll; padding-right: 20px; text-align: start" id="disclaimerText">Lorem ipsum .</div>`,
          confirmButtonText: `<span style="color: black">${translatorService.Translate('Accept')}</span>`,
          confirmButtonColor: '#FFD87F',
          allowOutsideClick: false,
          didRender: (m) => {
            let content = document.getElementById('swal2-content');
            let confirmButton = [...document.getElementsByClassName('swal2-confirm')][0];
            confirmButton.setAttribute('disabled', 'disabled');
            let disclaimerText = document.getElementById('disclaimerText');
            disclaimerText.addEventListener('scroll', function (e) {
              var windowHeight = "offsetHeight" in content ? content.offsetHeight : document.documentElement.offsetHeight;
              var docHeight = Math.max(disclaimerText.scrollHeight, disclaimerText.offsetHeight, content.clientHeight, content.scrollHeight, content.offsetHeight);
              windowBottom = windowHeight + disclaimerText.scrollTop;
