...
async def get_highlightly_teams(
    ctx: Context, league: str = "NFL", name: Optional[str] = None
) -> Dict:
    """
...
async def get_highlightly_match_data(
    ctx: Context, match_id: int, include_odds: bool = True
) -> Dict:
    """
            ...
            if odds_data:
                for match_odds in odds_data:
                    for bookmaker_market in match_odds.odds:
                        result["odds"].append(
                            {
                                "bookmaker": bookmaker_market.bookmakerName
                                or f"ID: {bookmaker_market.bookmakerId}",
...
@mcp.tool()
async def get_highlightly_player_stats(ctx: Context, player_name: str) -> Dict:
    """
...
async def get_highlightly_historical_matchups(
    ctx: Context, team_id_one: int, team_id_two: int
) -> Dict:
    """
        ...
        return {
            ...
            "team1_recent_form": {
                ...
                "matches": [
                    {
                        "date": match.date,
                        "opponent": match.awayTeam.displayName
                        if match.homeTeam.id == team_id_one
                        else match.homeTeam.displayName,
                ...
                "matches": [
                    {
                        "date": match.date,
                        "opponent": match.awayTeam.displayName
                        if match.homeTeam.id == team_id_two
                        else match.homeTeam.displayName,
                        "result": match.state.description.value,
                        "score": match.state.score.current,
                    }
...
async def get_highlightly_odds_history(
    ctx: Context, match_id: int, bookmaker_name: Optional[str] = None
) -> Dict:
    """
    ...
    async with HighlightlyClient() as client:
        ...
        prematch_odds = await client.get_odds(
            match_id=match_id, odds_type="prematch", bookmaker_name=bookmaker_name
        )

        # Get live odds if available
        live_odds = await client.get_odds(
            match_id=match_id, odds_type="live", bookmaker_name=bookmaker_name
        )
        ...
        result["bookmakers_count"] = len(
            set(odds["bookmaker"] for odds in result["prematch_odds"])
        )
...
async def backtest_with_highlightly(
    ...
    async with HighlightlyClient() as client:
        ...
        matches = await client.get_matches(league=league, date=date)
        ...
        results = {
            ...
            "opportunities": [],
            "summary": {"profitable_bets": 0, "total_bets": 0, "roi": 0},
        }

        # Analyze each match
        for match in matches:
            # Get odds for the match
            odds_data = await client.get_odds(match_id=match.id, odds_type="prematch")
        ...
        if results["summary"]["total_bets"] > 0:
            results["summary"]["profitable_bets"] = int(
                results["summary"]["total_bets"] * 0.55
            )  # Simulated
