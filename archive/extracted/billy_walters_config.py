...
if __name__ == "__main__":
    ...
    print(
        f"Response templates available: {sum(len(v['responses']) for v in INJURY_IMPACT_RESPONSES.values())}"
    )
    print(
        "\nKey insight: Markets underreact by {:.0f}% on average!".format(
            (1 - MARKET_ADJUSTMENTS["UNDERREACTION_FACTOR"]) * 100
        )
    )
