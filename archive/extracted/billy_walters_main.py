...
class BillyWaltersAnalyzer:
    ...
    def _enhance_with_specific_responses(self, analysis: Dict) -> Dict:
        ...
        for game in analysis["games"]:
            ...
            specific_response = self._generate_specific_response(
                response_template, game, total_impact
            )
    ...
    def _generate_specific_response(
        self, template: str, game: Dict, total_impact: float
    ) -> str:
        ...
        critical_count = game.get("home_injuries_count", 0) + game.get(
    ...
    def generate_betting_card(self, analysis: Dict) -> str:
        ...
        games_by_edge = sorted(
            analysis["games"], key=lambda x: abs(x.get("net_impact", 0)), reverse=True
        )
        ...
        moderate_plays = [
            g for g in games_by_edge if 2.0 <= abs(g.get("net_impact", 0)) < 3.0
        ]
        if moderate_plays:
        ...
        leans = [g for g in games_by_edge if 1.0 <= abs(g.get("net_impact", 0)) < 2.0]
        if leans:
...
def main():
    ...
    analyzer = BillyWaltersAnalyzer(
        project_dir="/home/omalleyandy/python_projects/billy-walters-sports-analyzer"
    )
