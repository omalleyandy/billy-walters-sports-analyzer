...
async def show_week_odds(target_dates=None):
    ...
    if not filtered_games:
        ...
        print()
        print(f"Available game dates (ET):")
        all_dates = set()
        for game_id, game_odds in games.items():
            if game_odds:
                commence_time = game_odds[0].get('commence_time', '')
    ...
    for i, (game_id, game_odds) in enumerate(sorted_games):
        ...
        other_spreads = {k: v for k, v in all_spreads.items()
        ...
        if sharp_spreads:
            ...
            avg_sharp = sum(d['line'] for d in sharp_spreads.values()) / len(sharp_spreads)
        ...
        if public_spreads:
            ...
            avg_public = sum(d['line'] for d in public_spreads.values()) / len(public_spreads)
        ...
        if all_spreads:
            ...
            if variance >= 1.0:
                ...
                print(f"   Best for {teams['away']}: {best_away[0]} ({best_away[1]['line']:+.1f})")
                print(f"   Best for {teams['home']}: {best_home[0]} ({best_home[1]['line']:+.1f})")
        ...
        if sharp_spreads and public_spreads:
            ...
            if abs(divergence) >= 0.5:
                ...
                if abs(divergence) >= settings.skills.market_analysis.alert_threshold:
                    print(f"   ** ALERT THRESHOLD EXCEEDED **")
                    if divergence > 0:
    ...
    print(f'Data source: The Odds API')
