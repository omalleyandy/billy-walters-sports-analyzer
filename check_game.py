    ...
    for idx, (game_id, game_odds) in enumerate(matching_games):
        ...
        if sharp_lines:
            ...
            for line in sharp_lines:
                print(f"  {line['book']:<20} {line['line']:+6.1f} ({line['price']:+4d})")
        ...
        if sharp_lines and public_lines:
            ...
            if abs(divergence) < 0.3:
                print(f"  Status: Lines are aligned (no edge)")
            elif abs(divergence) < settings.skills.market_analysis.alert_threshold:
                print(f"  Status: Minor divergence (watch)")
            else:
                print(f"  Status: SIGNIFICANT DIVERGENCE! (alert threshold exceeded)")
                if divergence > 0:
