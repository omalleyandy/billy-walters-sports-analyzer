...
class TestPregameScraperValidation(unittest.TestCase):
    ...
    def test_spread_parsing(self):
        ...
        # Find spread buttons
        spread_buttons = [
            btn for btn in self.sample_game["buttons"] if btn["id"].startswith("S")
        ]
    ...
    def test_total_parsing(self):
        ...
        # Find total buttons
        total_buttons = [
            btn for btn in self.sample_game["buttons"] if btn["id"].startswith("L")
        ]
        ...
        total_regex = re.compile(
            r"^([OU])\s+(\d+\.?\d?[½]?)\s+([+\-]\d{2,4})$", re.IGNORECASE
        )
    ...
    def test_date_time_parsing(self):
        ...
        self.assertTrue(
            date_regex.match(date_str), f"Date regex should match '{date_str}'"
        )

        # Time regex from pregame_odds_spider.py line 305
        time_regex = re.compile(r"^\d{1,2}:\d{2}\s+(AM|PM)$", re.IGNORECASE)
        self.assertTrue(
            time_regex.match(time_str), f"Time regex should match '{time_str}'"
        )
    ...
    def test_team_name_validation(self):
        ...
        self.assertTrue(
            team_name_regex.match(away_team),
            f"Away team should pass validation: '{away_team}'",
        )
        self.assertTrue(
            team_name_regex.match(home_team),
            f"Home team should pass validation: '{home_team}'",
        )
    ...
    def test_button_id_assignment(self):
        ...
        self.assertTrue(
            any(b["id"].startswith("S1_") for b in spread_buttons),
            "Should have S1 (away spread)",
        )
        self.assertTrue(
            any(b["id"].startswith("S2_") for b in spread_buttons),
            "Should have S2 (home spread)",
        )

        # L1 = over, L2 = under
        self.assertTrue(
            any(b["id"].startswith("L1_") for b in total_buttons),
            "Should have L1 (over)",
        )
        self.assertTrue(
            any(b["id"].startswith("L2_") for b in total_buttons),
            "Should have L2 (under)",
        )
