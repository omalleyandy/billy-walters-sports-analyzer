...
class TestEndToEndPipeline:
    ...
    def test_buffalo_houston_week12_integration(
        ...
        hou_adjusted = hou_context.current_power_rating + hou_context.total_injury_impact
    ...
    def test_batch_processing_all_teams(
        self, 
        team_builder, 
        real_nfl_teams
    ):
        ...
        elite_count = sum(1 for c in contexts if c.quality_tier == TeamQualityTier.ELITE)
        great_count = sum(1 for c in contexts if c.quality_tier == TeamQualityTier.GREAT)
...
class TestDataQuality:
    ...
    def test_minimum_data_completeness(self, team_builder, real_nfl_teams):
        ...
        for team_data in real_nfl_teams:
            ...
            populated = sum(1 for field in required_fields if getattr(context, field) is not None)
            completeness = (populated / len(required_fields)) * 100
            
            assert completeness >= 95.0, f"Team {context.team_name} only {completeness}% complete"
