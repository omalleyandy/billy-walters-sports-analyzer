...
async def test_validation_integration():
    ...
    weather_result = await validate_weather_data(
        {"temperature": 45, "wind_speed": 12, "precipitation_probability": 0.3}
    )
    ...
    try:
        validated_odds = await fetch_and_validate_odds(
            "NFL_2025_W10_TEST", mock_fetch_odds
        )
        print("   Success: Fetched and validated odds")
    ...
    try:
        await fetch_and_validate_odds("NFL_2025_W10_INVALID", mock_fetch_invalid_odds)
