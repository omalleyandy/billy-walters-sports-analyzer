...
class NetworkCapture:
    def __init__(self, url: str, method: str, status: int, headers: Dict[str, str], body: bytes | None):
        self.url = url
        self.method = method
        self.status = status
        self.headers = headers
        self.body = body
    def as_curl(self) -> str: return to_curl(self.method, self.url, self.headers, self.body)
    def as_fetch(self) -> str: return to_fetch(self.method, self.url, self.headers, self.body)

def _match(resp: Response, domain_hint: Optional[str]) -> bool:
    try:
        if resp.status != 200: return False
        if domain_hint and domain_hint not in resp.url: return False
