...
from nfl_team_data import NFL_TEAMS, get_team_by_name, is_division_game, get_all_divisions
    ...
    def calculate_travel_analysis(self, games: List[Dict]) -> Dict:
        ...
        for game in games:
            ...
            distance = self._haversine_distance(
                away_team["lat"], away_team["lon"],
                home_team["lat"], home_team["lon"]
            )
            ...
            travel_data[f"{away_abbr}@{home_abbr}"] = {
                ...
                "spread_adjustment": round(s_factor / 5.0, 2)  # 5 points = 1 spread point
            }
    ...
    def display_summary(self, data: Dict) -> None:
        ...
        for game_key, travel_info in data["travel"].items():
            if travel_info["category"] == "LONG":
                logger.info(f"\n{game_key}:")
                logger.info(f"  {travel_info['travel_from']} → {travel_info['travel_to']}")
                ...
                logger.info(f"  Spread Impact: {travel_info['spread_adjustment']} points")
