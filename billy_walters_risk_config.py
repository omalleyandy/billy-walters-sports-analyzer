...
class BillyWaltersRiskConfig:
    ...
    def validate_bet_size(
        self, bet_amount: float, bankroll: float, weekly_exposure: float = 0.0
    ) -> tuple[bool, str]:
        """
    ...
    def calculate_safe_bet_size(
        self, stars: float, bankroll: float, current_weekly_exposure: float = 0.0
    ) -> Dict[str, float]:
        """
...
if __name__ == "__main__":
    ...
    print(
        f"[LIST] Max Single Bet: {config.max_bet_percentage * 100:.1f}% = ${bankroll * config.max_bet_percentage:,.0f}"
    )
    print(
        f"[CHART] Max Weekly Exposure: {config.weekly_exposure_limit * 100:.1f}% = ${bankroll * config.weekly_exposure_limit:,.0f}"
    )
    ...
    for opp in week12_opportunities:
        ...
        if is_valid:
            ...
            print(
                f"   [*] BET: ${bet_info['bet_amount']:.0f} ({bet_info['bet_pct'] * 100:.1f}%)"
            )
