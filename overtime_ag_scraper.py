...
async def main():
    """Test the scraper"""
    scraper = OvertimeAgScraper(headless=True)  # Set to False to see browser (Windows only)
    ...
    try:
        ...
        if nfl_lines:
            for line in nfl_lines[:5]:  # Show first 5
                ...
                if line.spread:
                    print(f"  Spread: {line.spread:+.1f} ({line.spread_away_price}/{line.spread_home_price})")
                if line.total:
                    print(f"  Total: {line.total:.1f} (O: {line.over_price} / U: {line.under_price})")
                if line.away_ml:
                    print(f"  ML: {line.away_ml} / {line.home_ml}")
        else:
            print("\n⚠️  No betting lines found yet - parsing logic needs to be implemented")
            print("📁 Check the debug files (HTML and text) to understand page structure")
