...
class PowerRatingManager:
    ...
    def load_ratings(self, week: int = None):
        ...
        try:
            ...
            if week is None:
                ...
                # Get specific week
                week_data = next((h for h in data['history'] if h['week'] == week), None)
                if week_data:
    ...
    def update_from_game_result(self, game: GameResult):
        ...
        winner_new = self.update_with_90_10_formula(
            winner, 
            abs(adjusted_margin)
        )
        
        # Update loser (negative margin)
        loser_new = self.update_with_90_10_formula(
            loser,
            -abs(adjusted_margin)
        )
        ...
        print(f"  {winner}: {old_winner:.2f} → {winner_new:.2f} ({winner_new - old_winner:+.2f})")
        print(f"  {loser}: {old_loser:.2f} → {loser_new:.2f} ({loser_new - old_loser:+.2f})")
    ...
    def export_to_excel(self, filename: str = None):
        ...
        """
        if filename is None:
            filename = f"power_ratings_{self.sport}_{datetime.now().strftime('%Y%m%d')}.xlsx"
