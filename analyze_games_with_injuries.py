...
def calculate_injury_impact(injuries, bw_valuation):
    ...
    qb_injuries = [inj for inj in team_analysis['critical_injuries'] 
                   if inj['position'] == 'QB']
...
def main():
    ...
    for idx, analysis in enumerate(game_analysis, 1):
        ...
        print(f"Dome: {'Yes' if game.get('is_dome') else 'No'} | {game.get('venue_city')}, {game.get('venue_state')}")
        print()
        
        # Away team injuries (Billy Walters format)
        print(f"🏈 {away_team} ({game['away_record']})")
        print(f"   Billy Walters Impact: {away_impact['score']:.1f} point spread points")
        print(f"   Severity: {away_bw.get('severity', 'N/A')} | Confidence: {away_bw.get('confidence', 'N/A')}")
        
        if away_impact['qb_out'] > 0:
            print(f"   ⚠️  CRITICAL: QB OUT!")
        ...
        if away_bw.get('detailed_breakdown'):
            ...
            for inj in away_bw['detailed_breakdown'][:3]:  # Top 3
                print(f"      • {inj['name']} ({inj['position']}): {inj['explanation']}")
                print(f"        Impact: -{inj['impact']:.1f} pts (from base {inj['base_value']:.1f} pts)")
        
        # Position group crises
        if away_bw.get('position_group_crises'):
            print(f"   Position Group Impact:")
        ...
        print(f"   Billy Walters Impact: {home_impact['score']:.1f} point spread points")
        print(f"   Severity: {home_bw.get('severity', 'N/A')} | Confidence: {home_bw.get('confidence', 'N/A')}")
        ...
        print(f"{'─' * 80}")
        print(f"💰 BILLY WALTERS BETTING ANALYSIS:")
        print(f"{'─' * 80}")
        ...
        if abs(net_impact) >= 3.0:
            ...
            print(f"   Bet Sizing: 2-3% of bankroll")
            print(f"   Historical: 64% win rate with {abs(net_impact):.1f}+ point injury edge")
            ...
            print(f"   Bet Sizing: 1-2% of bankroll")
            print(f"   Historical: 58% win rate with {abs(net_impact):.1f}+ point edge")
        else:
            print(f"   ✓ BALANCED: Injuries relatively even between teams")
            print(f"   Action: NO EDGE from injuries - look for other factors")
        
        if analysis['total_impact'] >= 7.0:
            print(f"   ⚠️  WARNING: Combined {analysis['total_impact']:.1f} pts impact - high volatility game")
    ...
    qb_affected_games = sum(1 for a in game_analysis if a['away_impact']['qb_out'] > 0 or a['home_impact']['qb_out'] > 0)
    print(f"Games with QB out: {qb_affected_games}/{len(games)}")
    
    strong_edge_games = sum(1 for a in game_analysis if abs(a['away_impact']['score'] - a['home_impact']['score']) >= 3.0)
    print(f"Games with strong betting edge (3+ pts): {strong_edge_games}/{len(games)}")
    
    moderate_edge_games = sum(1 for a in game_analysis if 1.5 <= abs(a['away_impact']['score'] - a['home_impact']['score']) < 3.0)
