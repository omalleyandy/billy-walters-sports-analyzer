...
class PowerRatingsUpdater:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params) as response:
                    ...
                    # Verify MNF included
                    mnf_found = any(
                        (r["home_team"] == "Philadelphia Eagles" and r["away_team"] == "Dallas Cowboys") or
                        (r["home_team"] == "Dallas Cowboys" and r["away_team"] == "Philadelphia Eagles")
                        for r in results
                    )
    ...
    def get_week11_fallback(self) -> List[Dict]:
        ...
        return [
            ...
            {"home_team": "Washington Commanders", "away_team": "Philadelphia Eagles",
            ...
            {"home_team": "Minnesota Vikings", "away_team": "Tennessee Titans",
             "home_score": 23, "away_score": 13},
    ...
    def calculate_new_ratings(
        ...
        for game in results:
            ...
            if abs(home_change) > 1.0 or abs(away_change) > 1.0:
                logger.info(f"  {home_team:25} {home_old:+6.2f} → {home_new:+6.2f} ({home_change:+.2f})")
                logger.info(f"  {away_team:25} {away_old:+6.2f} → {away_new:+6.2f} ({away_change:+.2f})")
