...
def example_3_update_ratings():
    ...
    print(f"  Buffalo:     {buf_before:.2f} -> {buf_new:.2f} ({buf_new - buf_before:+.2f})")
    ...
    if our_spread:
        ...
        if abs(edge) >= 2.5:
            if edge > 0:
                ...
                print(f"    BET: Memphis (favorite has value)")
            else:
                print(f"\n  [YES] EDGE DETECTED: Tulane +{market_spread} has {abs(edge):.1f}-point edge")
                print(f"    BET: Tulane (underdog getting too many points)")
        else:
...
def example_6_season_simulation():
    ...
    for game in week1_games:
        ...
        result_str = f"{game.away_team} @ {game.home_team} ({game.away_score}-{game.home_score})"
