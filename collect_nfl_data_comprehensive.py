...
class NFLDataCollector:
        ...
        for game in schedule:
            ...
            if away_abbr in team_locations and home_abbr in team_locations:
                ...
                travel_analysis[f"{away_abbr}@{home_abbr}"] = {
                    ...
                    "s_factor_points": self._calculate_travel_s_factor(
                        distance, tz_diff
                    ),
                }
    ...
    async def calculate_factors(self, data: Dict) -> Dict:
        ...
        for game in data.get("schedule", []):
            ...
            travel_sf = (
                data.get("travel", {}).get(game_key, {}).get("s_factor_points", 0)
            )
...
async def main():
    ...
    try:
        ...
        print(
            f"Division Games: {len(data.get('factors', {}).get('division_games', []))}"
        )
