# Billy Walters Sports Analyzer - Agent Guide

**Status**: ✅ Phases 1-6 Complete (60% of Integration)

## 🚀 System Capabilities (Phases 1-6)

### Core Features
- ✅ **Billy Walters Methodology**: Power ratings, key numbers, injury valuation
- ✅ **AI-Enhanced Scraping**: Performance monitoring with Chrome DevTools patterns
- ✅ **MCP Server**: Claude Desktop integration with 6 analysis tools
- ✅ **Autonomous Agent**: 5-step reasoning chains with XGBoost/Random Forest
- ✅ **Slash Commands**: 12 interactive commands with REPL interface
- ✅ **Research Engine**: Multi-source data (AccuWeather, ProFootballDoc, Highlightly)
- ✅ **Bankroll Management**: Kelly Criterion with fractional Kelly safety

### Available Commands

#### Analysis Commands
```bash
# Single game analysis with research
uv run walters-analyzer analyze-game \
  --home "Team A" --away "Team B" \
  --spread -3.5 --research

# Interactive mode (NEW in Phase 6!)
uv run walters-analyzer interactive

# Single slash command
uv run walters-analyzer slash "/analyze Chiefs vs Bills -2.5"
```

#### Data Collection Commands
```bash
# AI-assisted scraping
uv run walters-analyzer scrape-ai --sport nfl

# Injury scraping
uv run walters-analyzer scrape-injuries --sport nfl

# Highlightly API
uv run walters-analyzer scrape-highlightly --endpoint matches --sport nfl
```

#### Monitoring Commands
```bash
# Sharp money monitoring
uv run walters-analyzer monitor-sharp --sport nfl --duration 60

# View scraped odds
uv run walters-analyzer view-odds --today
```

### Automation (Phase 7)

Use `.codex/super-run.ps1` for automated workflows:
```powershell
# Full daily workflow
.codex/super-run.ps1 -Task full-workflow -Sport nfl -Bankroll 10000

# Just collect data
.codex/super-run.ps1 -Task collect-data -Sport nfl

# Test system
.codex/super-run.ps1 -Task test-system
```

## Updating Personal `.env` Files

This repository uses `env.template` as the canonical list of environment variables. Follow this workflow whenever the template changes or you onboard a new environment:

1. **Back up your secrets.** Copy your personal `.env` to `.env.backup` before making changes so you can recover your values if needed.
2. **Review template updates.** Compare the new `env.template` with your backup to see which keys were added, removed, or renamed.
3. **Stage a merged file.** Duplicate `env.template` (e.g., `cp env.template .env.new`) to preserve its comments and ordering.
4. **Reapply real values.** Paste values from `.env.backup` into `.env.new` for existing keys. Provide values for new keys and remove obsolete ones only after confirming they are no longer required.
5. **Optional helper script.** If the list is long, adapt the Python snippet below to automate the merge:

   ```python
   from pathlib import Path

   def load_env(path: Path) -> dict[str, str]:
       data: dict[str, str] = {}
       for line in path.read_text().splitlines():
           stripped = line.strip()
           if not stripped or stripped.startswith("#") or "=" not in stripped:
               continue
           key, value = stripped.split("=", 1)
           data[key.strip()] = value.strip()
       return data

   template_path = Path("env.template")
   backup_path = Path(".env.backup")
   merged_path = Path(".env.new")

   template_vars = load_env(template_path)
   backup_vars = load_env(backup_path)

   merged_lines: list[str] = []
   for raw_line in template_path.read_text().splitlines():
       if not raw_line or raw_line.lstrip().startswith("#") or "=" not in raw_line:
           merged_lines.append(raw_line)
           continue
       key, _ = raw_line.split("=", 1)
       value = backup_vars.get(key, template_vars.get(key, ""))
       merged_lines.append(f"{key}={value}")

   merged_path.write_text("\n".join(merged_lines) + "\n")
   ```

6. **Validate.** Replace `.env` with `.env.new` once reviewed, then run the application or tests to ensure everything loads correctly. A quick sanity check is `pytest`, which exercises the parsers and spiders that depend on the environment. Never commit your personal `.env` or other files containing secrets.

## Documentation Organization

- Keep new Markdown artifacts under `docs/` (use `docs/guides/` for setup/how-to content and `docs/reports/` for investigations, briefs, and summaries).
- After a chat session generates `.md` files in the project root, run `python scripts/organize_docs.py` to sweep them into the right folder; this now also archives older reports to `docs/reports/archive/` so the active list stays short (pass `--skip-archive` if you need to opt out temporarily).
- Only `README.md` and this `AGENTS.md` should live in the repository root; everything else belongs in `docs/` so the workspace stays tidy.

## WSL + Windows PowerShell Integration Notes

This project often runs in a hybrid Windows + WSL2 workflow. Keep these guidelines handy when switching between shells or diagnosing integration hiccups.

### Command Separation Rules
- Windows PowerShell commands run from `PS C:\Users\omall\Documents\python_projects\billy-walters-sports-analyzer`.

### Opening WSL Paths in Windows Explorer
- From Ubuntu (bash):
  - `explorer.exe .`
  - `explorer.exe omalleyandy@In-The-Drone:~$`
  - `explorer.exe "/mnt/c/Users/<user>"`
- From PowerShell (preferred to fallback):
  - `explorer.exe "\\wsl.localhost\Ubuntu\home\omalleyandy\billy-walters-sports-analyzer"`
  - `explorer.exe "\\wsl.localhost\Ubuntu\home\omalleyandy\"`

### Checking WSL Mount Health (Ubuntu)
```bash
mount | grep -E '/mnt/c|drvfs'
```
Expected output should include entries like:
```bash
C:\ on /mnt/c type 9p (rw,aname=drvfs,...)
```
This confirms drives are mounted with normal read/write permissions and WSL is healthy.

### Restarting WSL Cleanly (PowerShell, Admin)
```powershell
wsl --shutdown
Restart-Service LxssManager
wsl
```
This resets the virtualized network bridge and resolves most UNC lookup failures.

### Fixing UNC Path Issues
If Windows Explorer reports “Attempt to access invalid address”, run (PowerShell, Admin):
```powershell
wsl --shutdown
Restart-Service LxssManager
```
If problems persist:
```powershell
netsh winsock reset
Restart-Computer
```

### Mapping WSL Paths as Drive Letters (PowerShell only)
```powershell
New-PSDrive -Name W -PSProvider FileSystem -Root "\\wsl.localhost\Ubuntu\home\omalleyandy\billy-walters-sports-analyzer" -Persist
Set-Location W:
```

### Verifying WSL State
```powershell
wsl -l -v
```
Expected output: `Ubuntu    Stopped/Running   2`.

**Key Principle:** WSL mounts are typically healthy; UNC path lookup failures are usually Windows networking hiccups, not permissions issues.

These instructions apply to the entire repository unless overridden by a more specific `AGENTS.md` in a subdirectory.
