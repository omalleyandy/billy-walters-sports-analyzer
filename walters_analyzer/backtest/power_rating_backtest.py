...
from ..valuation.power_ratings import PowerRatingSystem, GameResult, initialize_nfl_ratings
...
class PowerRatingBacktest:
    ...
    def run_backtest(
        self,
        games: List[Dict],
        week_labels: Optional[List[int]] = None
    ) -> BacktestResult:
        """
        ...
        for i, game_data in enumerate(games):
            ...
            correct_winner = (
                (predicted_spread > 0 and actual_margin > 0) or  # Predicted home win, home won
                (predicted_spread < 0 and actual_margin < 0) or  # Predicted away win, away won
                (predicted_spread == 0)  # Predicted tie (rare)
            )
        ...
        biggest_movers = sorted(rating_changes, key=lambda x: abs(x[1]), reverse=True)[:10]
    ...
    def generate_report(self, result: BacktestResult, output_file: Optional[Path] = None) -> str:
        """
        ...
        lines.append("=" * 80)
        lines.append(f"Winner Prediction: {result.correct_winner_pct:.1%} "
                    f"({sum(p.correct_winner for p in result.predictions)}/{result.total_games})")
        lines.append(f"ATS Record: {result.ats_record[0]}-{result.ats_record[1]}-{result.ats_record[2]} "
                    f"({result.ats_win_pct:.1%})")
        lines.append(f"Mean Absolute Error: {result.mean_absolute_error:.2f} points")
        lines.append(f"Median Absolute Error: {result.median_absolute_error:.2f} points")
        lines.append("")
        ...
        if result.weekly_stats:
            ...
            lines.append("=" * 80)
            lines.append(f"{'Week':<8} {'Games':<8} {'Winner%':<12} {'ATS%':<12} {'Avg Error'}")
            lines.append("-" * 80)
        ...
        lines.append("=" * 80)
        worst_preds = sorted(result.predictions, key=lambda x: x.prediction_error, reverse=True)[:10]
        ...
        lines.append("=" * 80)
        top_teams = sorted(result.final_ratings.items(), key=lambda x: x[1], reverse=True)[:10]
