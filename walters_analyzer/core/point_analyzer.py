...
class PointAnalyzer:
    ...
    def evaluate(self, projected_spread: float, market_spread: float) -> List[KeyNumberAlert]:
        """
        ...
        for number in self.key_numbers:
            if lower <= number <= upper:
                direction = "toward the favorite" if movement_toward_favorite else "toward the underdog"
                alerts.append(
                    KeyNumberAlert(
                        number=number,
                        crossed=True,
