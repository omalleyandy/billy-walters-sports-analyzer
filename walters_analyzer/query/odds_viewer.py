    ...
    def get_upcoming_games(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        for game in self.games:
            ...
            if (
                ...
                and event_date >= today.isoformat()
            ):
                filtered.append(game)
    ...
    def compare_lines(self, team_name: str):
        ...
        for game in sorted(games, key=lambda g: g.get("event_date", "")):
            ...
            if spread_line:
                ...
                print(
                    f"{event_date} {event_time:12s} | {location}{opponent:25s} | {sign}{line:5.1f} ({price:+4d})"
                )
            else:
                print(
                    f"{event_date} {event_time:12s} | {location}{opponent:25s} | No spread available"
                )
...
Examples:
  ...
  python odds_viewer.py --sport nfl --export nfl_odds.csv
    ...
    parser.add_argument(
        "--upcoming", type=int, metavar="DAYS", help="Show games in next N days"
    )
    parser.add_argument("--team", help="Filter by team name")
    parser.add_argument("--compare", metavar="TEAM", help="Compare lines for a team")
    parser.add_argument("--summary", action="store_true", help="Show summary only")
    parser.add_argument(
        "--brief", action="store_true", help="Brief output (no detailed odds)"
    )
    ...
    if args.file:
        ...
        if count > 0:
            latest_file = sorted(
                Path(args.data_dir).glob("overtime-live-*.jsonl"), reverse=True
            )[0]
