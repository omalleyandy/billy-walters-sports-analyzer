...
def main():
    ...
    try:
        ...
        if settings.global_config.debug_mode:
            ...
            logger.debug(
                f"Skills enabled: market_analysis={settings.skills.market_analysis.enabled}, "
    ...
    wk.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview entries and gating without 'placing'",
    )
    wk.add_argument(
        "--show-bankroll",
        action="store_true",
        help="Show bankroll percentages and amounts",
    )
    wk.add_argument(
        "--bankroll",
        type=float,
        default=10000.0,
        help="Starting bankroll amount (default: $10,000)",
    )

    # analyze-game: Analyze a single game with full Billy Walters methodology
    analyze = sub.add_parser(
        "analyze-game", help="Analyze a game using Billy Walters methodology"
    )
    ...
    analyze.add_argument(
        "--spread", type=float, help="Current spread (home team perspective)"
    )
    analyze.add_argument("--total", type=float, help="Current total")
    analyze.add_argument(
        "--home-price", type=int, default=-110, help="Home spread price (default: -110)"
    )
    analyze.add_argument(
        "--away-price", type=int, default=-110, help="Away spread price (default: -110)"
    )
    ...
    analyze.add_argument(
        "--research", action="store_true", help="Fetch live injury/weather data"
    )

    # scrape-ai: Scrape odds with AI-assisted performance monitoring (NEW!)
    scrape_ai = sub.add_parser(
        "scrape-ai",
        help="Scrape odds with AI performance monitoring (requires MCP chrome-devtools)",
    )
    scrape_ai.add_argument(
        "--sport", choices=["nfl", "ncaaf"], default="nfl", help="Sport to scrape"
    )
    scrape_ai.add_argument(
        "--output-dir",
        default="data/odds_chrome",
        help="Output directory for scraped data",
    )
    scrape_ai.add_argument(
        "--no-ai-report", action="store_true", help="Skip AI insights report"
    )

    # interactive: Interactive slash command mode (NEW!)
    interactive = sub.add_parser(
        "interactive", help="Enter interactive slash command mode"
    )
    ...
    slash.add_argument(
        "command",
        help="Slash command to execute (e.g., '/analyze Chiefs vs Bills -2.5')",
    )
    slash.add_argument(
        "--bankroll",
        type=float,
    ...
    highlightly = sub.add_parser(
        "scrape-highlightly", help="Scrape NFL/NCAA data from Highlightly API"
    )
    highlightly.add_argument(
        "--endpoint",
        choices=[
            "teams",
            "matches",
    ...
    highlightly.add_argument(
        "--date", help="Date filter (YYYY-MM-DD format, for matches/highlights)"
    )
    highlightly.add_argument(
        "--match-id", type=int, help="Match ID (for match details, odds, lineups)"
    )
    highlightly.add_argument(
        "--team-id", type=int, help="Team ID (for statistics, last-five games)"
    )
    highlightly.add_argument(
        "--team-id-two", type=int, help="Second team ID (for head-to-head)"
    )
    highlightly.add_argument("--name", help="Search by name (for teams/players)")
    highlightly.add_argument("--season", type=int, help="Season year filter")
    highlightly.add_argument(
        "--output-dir",
        default=None,
    ...
    vo.add_argument(
        "--data-dir",
        default="data/overtime_live",
        help="Directory containing scraped data",
    )
    vo.add_argument("--file", help="Specific JSONL file to load")
    vo.add_argument(
        "--sport", choices=["nfl", "college_football"], help="Filter by sport"
    )
    vo.add_argument("--date", help="Filter by date (YYYY-MM-DD)")
    vo.add_argument("--today", action="store_true", help="Show today's games")
    vo.add_argument(
        "--upcoming",
        type=int,
        metavar="DAYS",
        help="Show games in next N days (default: 7)",
        const=7,
        nargs="?",
    )
    vo.add_argument("--team", help="Filter by team name (partial match)")
    vo.add_argument(
        "--compare", metavar="TEAM", help="Compare lines for a team across games"
    )
    vo.add_argument(
        "--summary", action="store_true", help="Show summary only (no detailed odds)"
    )
    vo.add_argument(
        "--brief", action="store_true", help="Brief output (game info only, no odds)"
    )
    vo.add_argument(
        "--export", metavar="FILE", help="Export filtered results to CSV file"
    )

    # monitor-sharp: Monitor market for sharp money movements
    monitor = sub.add_parser(
        "monitor-sharp", help="Monitor market for sharp money movements"
    )
    ...
    monitor.add_argument("--game-id", help="Specific game ID to monitor (optional)")
    monitor.add_argument(
        "--duration",
        type=int,
        default=120,
    ...
    monitor.add_argument(
        "--test", action="store_true", help="Test connection to The Odds API"
    )
    ...
    if args.cmd == "wk-card":
        ...
        print(
            summarize_card(
                ...
                analyzer=analyzer,
                show_bankroll=args.show_bankroll,
            )
        )
        ...
        async def run_analysis():
            ...
            if args.research:
                ...
                try:
                    snapshot = await engine.gather_for_game(
                        args.home, args.away, venue=args.venue, date=game.kickoff
                    )
                    ...
                    if snapshot.weather:
                        print(
                            f"[+] Weather: {snapshot.weather.get('conditions', 'N/A')}"
                        )
                    print()
            ...
            # Display results
            print("=" * 80)
            print("BILLY WALTERS GAME ANALYSIS")
            print("=" * 80)
            print()
            print(f"Matchup:  {args.away} @ {args.home}")
            if odds:
                print(
                    f"Spread:   {args.home} {result.market_spread:+.1f} ({args.home_price:+d}/{args.away_price:+d})"
                )
            print()
        ...
        if args.live:
            ...
            print("Starting live betting scraper for overtime.ag...")
        else:
        ...
        cmd = [
            ...
            spider_name,
            "-s",
            f"OVERTIME_OUT_DIR={args.output_dir}",
        ]
        ...
        try:
            ...
            print(
                f"ERROR: Scraping failed with exit code {e.returncode}", file=sys.stderr
            )
            sys.exit(e.returncode)
        except FileNotFoundError:
            print(
                "ERROR: scrapy command not found. Make sure scrapy is installed.",
                file=sys.stderr,
            )
        ...
        cmd = [
            ...
            "espn_injuries",
            "-s",
            f"FEEDS[{args.output_dir}/{filename_prefix}-%(time)s.jsonl]=jsonlines",
            "-s",
            f"FEEDS[{args.output_dir}/{filename_prefix}-%(time)s.parquet]=parquet",
        ]
        ...
        try:
            ...
            print(
                f"ERROR: Injury scraping failed with exit code {e.returncode}",
                file=sys.stderr,
            )
            sys.exit(e.returncode)
        except FileNotFoundError:
                    ...
                    async with HighlightlyClient() as client:
                        ...
                        if args.endpoint in ["matches", "all"]:
                            ...
                            if matches:
                                ...
                                save_to_jsonl(
                                    matches, "matches", sport_key, output_dir, extra
                                )
                        ...
                        if args.endpoint in ["odds", "all"]:
                            ...
                            if odds:
                                extra = (
                                    str(args.match_id)
                                    if args.match_id
                                    else (args.date or "latest")
                                )
                                save_to_jsonl(
                                    odds,
                                    f"odds-{args.odds_type}",
                                    sport_key,
                                    output_dir,
                                    extra,
                                )
                        ...
                        if args.endpoint in ["bookmakers", "all"]:
                            ...
                            if bookmakers:
                                save_to_jsonl(
                                    bookmakers, "bookmakers", sport_key, output_dir
                                )
                        ...
                        if args.endpoint in ["players", "all"]:
                            ...
                            if players:
                                ...
                                save_to_jsonl(
                                    players, "players", sport_key, output_dir, extra
                                )
                        ...
                        if args.endpoint == "head-to-head":
                            ...
                            if h2h:
                                save_to_jsonl(
                                    ...
                                    sport_key,
                                    output_dir,
                                    f"{args.team_id}-vs-{args.team_id_two}",
                                )
                                total_saved += len(h2h)
        ...
        if args.file:
            ...
            if count > 0:
                ...
                jsonl_files = sorted(
                    data_path.glob("overtime-live-*.jsonl"), reverse=True
                )
                if jsonl_files:
        ...
        async def run_ai_scraping():
            ...
            result = await scraper.scrape_overtime_odds(
                sport=args.sport, save_ai_report=not args.no_ai_report
            )
            ...
            if result.get("ai_insights"):
                ...
                perf_score = insights.get("performance_analysis", {}).get(
                    "performance_score", 0
                )
                ...
                if perf_score < 70:
                    print(
                        "  [WARNING] Low performance score - review AI recommendations"
                    )
                elif perf_score < 85:
                ...
                if not insights.get("extraction_success"):
                    ...
                    if debug_info.get("potential_causes"):
                        ...
                        for cause in debug_info["potential_causes"]:
                            print(
                                f"  • {cause.get('cause')} (likelihood: {cause.get('likelihood')})"
                            )
            ...
            print(
                f"  Avg Performance: {session_report['summary']['avg_performance_score']:.1f}/100"
            )

            if session_report["summary"]["common_issues"]:
                print(
                    f"  Common Issues: {', '.join(session_report['summary']['common_issues'][:3])}"
                )
        ...
        import asyncio
        from walters_analyzer.slash_commands import (
            SlashCommandHandler,
            interactive_mode,
        )
