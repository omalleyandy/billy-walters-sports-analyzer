...
from .core import BillyWaltersAnalyzer, BankrollManager
...
class SlashCommandHandler:
    ...
    async def cmd_analyze(self, args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            vs_idx = next((i for i, x in enumerate(args) if x in ['vs', '@', 'v']), None)
            if vs_idx is None:
                raise ValueError("Missing 'vs' or '@' separator")
            
            home_team = ' '.join(args[:vs_idx])
            spread_idx = next((i for i, x in enumerate(args[vs_idx+1:]) 
                              if x.startswith('-') or x.startswith('+')), None)
            ...
            return {
                ...
                'data': {
                    ...
                    'key_numbers': [alert.description for alert in analysis.key_number_alerts]
                },
                'ai_insights': {
                    'type': 'game_analysis',
        ...
        try:
            if topic == 'slate':
                ...
                return {
                    ...
                    'ai_insights': {
                        'impact': self._weather_impact_analysis(weather) if weather else None
                    }
                }
    ...
    async def cmd_market(self, args: List[str]) -> Dict[str, Any]:
        ...
        if action == 'monitor':
            ...
            return {
                ...
                'data': {
                    'active_alerts': [],
                    'message': 'No active market alerts'
                }
            }
            ...
            return {
                ...
                'data': {
                    'bankroll': {
                        ...
                        'change': self.analyzer.bankroll.bankroll - self.analyzer.bankroll.initial_bankroll,
    ...
    async def cmd_help(self, args: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            'data': {
                'available_commands': {
                    cmd: get_command_desc(self.commands[cmd])
    ...
    async def cmd_debug(self, args: List[str]) -> Dict[str, Any]:
        ...
        if target == 'last' and self.command_history:
            ...
            return {
                ...
                'ai_insights': {
                    'diagnosis': 'Command executed successfully' if last_cmd['result'] == 'success' else 'Command failed',
    ...
    def _explain_confidence(self, analysis) -> str:
        ...
        if edge >= 3.0:
            ...
            return f"Slight edge: {edge:.1f} pt edge is minimal. Historical win rate: 54%"
        else:
    ...
    def _assess_risk(self, analysis) -> Dict[str, Any]:
        ...
        return {
            ...
            'injury_risk': 'high' if len(analysis.home_report.critical_players) > 2 else 'low',
            'key_number_risk': 'alert' if analysis.key_number_alerts else 'none',
            'recommendation': 'Proceed with caution' if abs(analysis.edge) < 2.0 else 'Strong bet'
        }
    ...
    def _suggest_optimizations(self, analysis) -> List[str]:
        ...
        if analysis.recommendation.stake_pct == 0:
            suggestions.append("Edge too small for betting. Wait for better line or skip this game.")
        
        if analysis.key_number_alerts:
            suggestions.append(f"Key number detected: {analysis.key_number_alerts[0].description}. Consider timing your bet carefully.")
        
        if abs(analysis.injury_advantage) > 2.0:
            suggestions.append(f"Significant injury advantage ({analysis.injury_advantage:+.1f} pts). Verify injury statuses before betting.")
    ...
    def _injury_recommendations(self, injuries: List[Dict]) -> List[str]:
        ...
        if qb_injuries:
            recs.append(f"QB injury detected: {qb_injuries[0].get('name')} - major concern")
