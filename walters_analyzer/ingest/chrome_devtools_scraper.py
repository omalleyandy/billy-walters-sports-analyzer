...
class ChromeDevToolsOddsExtractor:
    ...
    def extract_games_from_snapshot(self, snapshot_text: str) -> List[Dict]:
        ...
        while i < len(lines):
            ...
            date_match = re.search(
                r'StaticText "((?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s+\w+\s+\d+)"', line
            )
            if date_match:
    ...
    def _extract_game_pair(self, lines: List[str], start_idx: int) -> Optional[Dict]:
        ...
        try:
            ...
            # Build game object
            iso_date, time_with_tz = parse_date_time(
                self.current_date or "", self.current_time or ""
            )
            ...
            game = {
                ...
                "teams": {"away": away_data["team"], "home": home_data["team"]},
                "markets": {
                    "spread": {
                        "away": away_data["spread"],
                        "home": home_data["spread"],
                    },
                    "total": {"over": away_data["total"], "under": home_data["total"]},
                    "moneyline": {
            ...
            game["game_key"] = self._generate_game_key(
                away_data["team"], home_data["team"], iso_date
            )
    ...
    def _generate_game_key(
        self, away_team: str, home_team: str, date: Optional[str]
    ) -> str:
